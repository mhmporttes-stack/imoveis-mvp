...
export async function listLeadDistributionDashboard() {
  ...
  const [{ data: users, error: usersError }, { data: registrations, error: registrationsError }, { data: state, error: stateError }] = await Promise.all([
    supabase.from("admin_users").select("id, name, email, role, status, lead_distribution_enabled, lead_distribution_position").in("role", ["admin", "broker", "associate"]),
...
export async function reorderLeadDistribution(order = []) {
  ...
  const supabase = getSupabaseAdminClient();
  const { data, error } = await supabase.from("admin_users").select("id").in("id", ids).in("role", ["admin", "broker", "associate"]);
