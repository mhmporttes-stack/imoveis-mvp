...
export async function getDailyReport(params = {}) {
  ...
  const registrations = await loadRegistrationsInRange(supabase, range);
  const history = await loadStatusHistoryInRange(supabase, range);
...
async function loadRegistrationsInRange(supabase, range) {
...
async function loadStatusHistoryInRange(supabase, range) {
  const { data, error } = await supabase
