...
export async function getDailyReport(params = {}, auth = null) {
  ...
  const scopedClientIds = await loadScopedClientIds(supabase, auth);

  const registrations = await loadRegistrationsInRange(supabase, range, auth);
...
async function loadRegistrationsInRange(supabase, range, auth = null) {
  let query = supabase
...
async function loadScopedClientIds(supabase, auth = null) {
  if (auth?.profile?.role !== "broker" || !auth.profile.id) return null;
