...
async function listCandidateClients(rule, limit) {
  let query = getSupabaseAdminClient()
    ...
    .select("id, full_name, phone, phone_normalized, status, responsible_user_id, created_at, updated_at, last_status_change_at, last_whatsapp_contact_at, scheduled_activity_at, scheduled_activity_type, scheduled_activity_note, scheduled_activity_completed_at")
...
function validateRule(payload) {
  ...
  for (const action of rule.actions) {
    ...
    if (action.type === "send_trigger_email") {
      const recipientEmail = cleanText(action.recipientEmail, 254);
      const subject = cleanText(action.subject, 160);
      if (!recipientEmail || !recipientEmail.includes("@")) throw new Error("Informe o e-mail destinatário do gatilho.");
...
async function sendTriggerEmail(action, client) {
  ...
  const to = cleanText(action.recipientEmail, 254);
  const broker = await resolveResponsibleBroker(client.responsible_user_id);
  const variables = buildEmailVariables(client, broker);
  let subject = renderTemplate(cleanText(action.subject, 160), variables);
  if (!/\{corretor_(?:codigo|nome)\}/i.test(action.subject || "")) subject = `${subject} ${broker.code}`;
...
function buildEmailVariables(client, broker) {
  const activity = validDate(client.scheduled_activity_at);
  return {
