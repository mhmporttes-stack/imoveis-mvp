...
async function sendTriggerEmail(action, client) {
  ...
  if (/^RECEBEU NOVO CLIENTE\b/i.test(subjectTemplate)) {
    subject = `RECEBEU NOVO CLIENTE — ${broker.name}`;
  } else if (!/\{corretor_(?:codigo|nome)\}/i.test(action.subject || "")) {
    subject = `${subject} ${broker.name}`;
  }
...
async function resolveResponsibleBroker(responsibleUserId) {
  ...
  const { data, error } = await getSupabaseAdminClient()
    ...
    .maybeSingle();
  if (error) throw error;
  const name = cleanText(data?.name, 80) || "Sem corretor";
  return {
    name,
    code: cleanText(data?.simulation_ref, 80).toUpperCase() || name.toUpperCase().replace(/[^A-Z0-9]+/g, "_"),
