...
async function runRule(rule, limit) {
  ...
  for (const client of clients) {
    ...
    if (returnsClientToRoundRobin(rule) && client.last_whatsapp_contact_at) continue;
...
async function listCandidateClients(rule, limit) {
  let query = getSupabaseAdminClient()
    ...
    .select("id, full_name, phone, phone_normalized, status, responsible_user_id, previous_responsible_user_id, responsible_changed_at, created_at, updated_at, last_status_change_at, last_whatsapp_contact_at, scheduled_activity_at, scheduled_activity_type, scheduled_activity_note, scheduled_activity_completed_at")
