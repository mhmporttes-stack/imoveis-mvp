...
async function sendTriggerEmail(action, client) {
  ...
  const subjectBase = cleanText(action.subject, 120);
  const brokerName = await resolveResponsibleBrokerName(client.responsible_user_id);
  const subject = cleanText(`${subjectBase} ${brokerName}`, 160);
  ...
  const response = await fetch(RESEND_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      to,
      subject,
      text: "Gatilho automático do CRM.",
      html: "<p>Gatilho automático do CRM.</p>"
    })
  });
  if (!response.ok) {
...
async function resolveResponsibleBrokerName(responsibleUserId) {
  if (!responsibleUserId) return "SEM CORRETOR";
  const { data, error } = await getSupabaseAdminClient()
    .from("admin_users")
    .select("name")
    ...
    .maybeSingle();
  if (error) throw error;
  return cleanText(data?.name, 80) || "SEM CORRETOR";
}
