...
async function executeAction(action, client) {
  ...
  if (action.type === "send_push") {
    // Mesmo destinatário das demais ações desta regra: o corretor responsável
    // pelo cliente no momento do gatilho (client.responsible_user_id já foi
    // resolvido ao carregar os candidatos da regra). Best-effort: se o
    // corretor não tiver nenhuma assinatura de push ativa, não falha a regra.
    if (!client.responsible_user_id) throw new Error("O push não possui destinatário válido.");
    await sendPushToUser(client.responsible_user_id, {
      title: cleanText(action.title, 160) || "Automação do CRM",
