...
export async function listCaptacoes() {
...
export async function getCaptacao(id) {
  const supabase = getCaptacoesClient();
  const { data, error } = await supabase
    .from("captacoes")
    .select("*")
  ...
  if (error) throw error;
  return data ? rowToCaptacao(data) : null;
}
...
export async function createCaptacao(payload) {
  ...
  const { data, error } = await supabase
    .from("captacoes")
    .insert(captacaoToRecord(parsed))
    .select("*")
    .single();
...
export async function updateCaptacao(id, payload = {}) {
  const parsed = captacaoAdminUpdateSchema.parse(payload);
  const record = partialCaptacaoToRecord(parsed);

  if (!Object.keys(record).length) return getCaptacao(id);
...
export async function deleteCaptacao(id) {
  const supabase = getCaptacoesClient();
...
export async function createPropertyDraftFromCaptacao(id) {
  ...
  const property = await createProperty(captacaoToPropertyDraft(captacao));
  const supabase = getCaptacoesClient();
