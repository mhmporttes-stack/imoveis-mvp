...
// IMPORTANTE: disponibilizar um contato para a Meta Diária NÃO é a mesma
// coisa que ele virar cliente. Por isso esta função NUNCA mais toca
// simulation_registrations — só cria a rodada (client_id ainda nulo). O
// registro de cliente só é criado no primeiro toque real (ver
// registerDailyGoalAttempt), que é a "ação válida de atendimento" que
// legitima a criação/atribuição de um cliente. Até lá, o contato existe
// só em prospecting_contacts + daily_goal_rounds — exatamente como um
// contato "disponibilizado para prospecção" que ainda não foi trabalhado.
...
// "Meta diária é diária": produção = só o que foi trabalhado NAQUELE dia
// (pelo menos 1 tentativa real, ou conversão). Reserva sem trabalho não
// entra no numerador — é exatamente o que volta pra Prospecção abaixo.
async function computeDailyGoalDayDone(brokerId, day) {
  const { data, error } = await db()
    .from("daily_goal_rounds")
    .select("id, attempt_count, status")
    .eq("broker_id", brokerId)
    .eq("round_started_at", day);
  if (error) throw error;
  return (data || []).filter((round) => round.status === "converted" || round.attempt_count >= 1).length;
}

// Devolve à Prospecção todo contato reservado naquele dia que NUNCA foi
// trabalhado (rodada ainda ativa, zero tentativas) — nunca mexe em rodadas
// com pelo menos 1 tentativa: essas continuam sua cadência normal de 2º/3º
// contato nos dias seguintes, comportamento intencional já existente.
async function releaseUnworkedRoundsForDay(brokerId, day) {
  const { data: rounds, error } = await db()
    .from("daily_goal_rounds")
    .select("id, prospecting_contact_id, client_id")
    .eq("broker_id", brokerId)
    .eq("round_started_at", day)
    .eq("status", "active")
    .eq("attempt_count", 0);
  ...
  for (const round of rounds) {
    const { data: ended } = await db()
      .from("daily_goal_rounds")
      .update({ status: "ended_no_conversion", ended_at: now })
      .eq("id", round.id)
      .eq("status", "active")
      .select("id")
      .maybeSingle();
    if (!ended) continue; // já processado por uma execução concorrente/anterior — idempotente

    await runUpdate(db().from("prospecting_contacts")
      .update({ status: "available", assigned_user_id: null, available_after: null, queue_sort_at: now, updated_at: now })
      .eq("id", round.prospecting_contact_id));

    // Só existe client_id aqui para rodadas antigas, criadas antes desta
    // correção (o código atual não materializa mais cliente na reserva) —
    // mantido por segurança/retrocompatibilidade. Reverte exatamente como o
    // fim de cadência por 3 tentativas já faz (arquiva + desatribui), nunca
    // apaga o registro.
    if (round.client_id) {
      await runUpdate(db().from("simulation_registrations")
        .update({ responsible_user_id: null, status: CLIENT_STATUS.ARCHIVED, last_status_change_at: now })
        .eq("id", round.client_id)
        .eq("status", CLIENT_STATUS.AWAITING_RETURN));
    }

    await logHistory(round.prospecting_contact_id, round.client_id, brokerId, "daily_goal_round_ended", { attempt: 0, reason: "day_closed_not_worked" });
  }
  return rounds.length;
}

// Fecha em definitivo o dia `day` de um corretor: registra o resultado final
// (previsto/realizado/percentual/bateu-ou-não) em daily_goals e devolve os
// contatos não trabalhados à Prospecção. Idempotente por construção: o
// UPDATE só afeta a linha se closed_at ainda for nulo (guarda de
// concorrência/repetição — rodar duas vezes não duplica nada, a segunda
// chamada simplesmente não encontra linha para fechar de novo).
async function closeDailyGoalDay(brokerId, day, quota) {
  const done = await computeDailyGoalDayDone(brokerId, day);
  const percent = quota ? Math.round((done / quota) * 100) : 0;
  const goalMet = quota > 0 && done >= quota;

  const { data: closed, error } = await db()
    .from("daily_goals")
    .update({ done_count: done, percent, goal_met: goalMet, closed_at: new Date().toISOString() })
    .eq("broker_id", brokerId)
    .eq("goal_date", day)
    .is("closed_at", null)
    .select("id")
  ...
  await releaseUnworkedRoundsForDay(brokerId, day);
...
export async function getDailyGoalPerformance(params, auth) {
  ...
  // Período de 1 dia (Hoje/Ontem): previsto usa total_due, o total "ao vivo"
  // daquele dia específico (cota nova + retorno de 2º/3º contato) — mantido
  // como sempre foi. Períodos de mais de 1 dia (7/30 dias, personalizado):
  // previsto é a SOMA do new_quota de cada dia já gerado (congelado na hora
  // da geração daquele dia — nunca recalculado com a config atual). Um dia
  // em que o corretor nunca abriu a Meta Diária simplesmente não contribui
  // nada ao previsto (não existia meta gerada naquele dia) — nunca inventa
  // um valor usando a cota vigente de hoje, que é exatamente a recalculação
  // retroativa que a regra da Meta Diária proíbe (ver migration
  // 20260916_daily_goal_closing.sql: daily_goals já congela o resultado por
  // dia e nunca deve ser reescrito por uma mudança de configuração futura).
  const daysInRange = countCalendarDaysInclusive(range.startDate, range.endDate);
  const isMultiDay = daysInRange > 1;
  ...
  for (const row of goals || []) {
    ...
    entry.previstas += isMultiDay ? (row.new_quota || 0) : (row.total_due || 0);
    if (row.closed_at) {
