...
import { getDailyGoalWalletStatus, getDailyGoalWalletConfig, updateDailyGoalWalletConfig } from "./daily-goal-wallet";
...
async function ensureDailyGoalGenerated(auth, today) {
  ...
  const { data: claimed, error: claimError } = await db().rpc("claim_daily_goal_contacts", { p_broker_id: brokerId, p_quota: quota, p_today: today });
...
async function buildDailyGoalSnapshot(brokerId, today) {
  ...
  // "total"/"done" abaixo continuam existindo só para os CARDS de cada grupo
  // (quantos pendentes vs. concluídos em cada etapa) — nunca mais para o
  // percentual da meta, que agora é SEMPRE ações reais de hoje / cota do dia
  // (ver computeDailyGoalRealizedToday), sem teto. Bater 40 ações com cota 20
  // deve mostrar 200%, não 100% — misturar "quanto ainda cabe na carteira"
  // com "quanto já foi trabalhado hoje" é exatamente a confusão meta×
  // capacidade que este pente-fino corrige.
  ...
  // A meta exibida ao corretor acompanha a carteira ativa real. A cota
  // global continua servindo apenas para distribuir novos contatos; ela nao
  // pode transformar uma carteira de 36 clientes em uma meta artificial de
  // 20. Assim o painel mostra 9/36 e so chega a 100% em 36/36.
  const quota = wallet.current || goalRow?.new_quota || 0;
  const realizedToday = await computeDailyGoalRealizedToday(brokerId, today);
  const percent = quota ? Math.round((realizedToday / quota) * 100) : 0;
...
export async function getDailyGoalPerformance(params, auth) {
  ...
  // Previsto = SEMPRE a soma do new_quota de cada dia já gerado (congelado na
  // hora da geração daquele dia — nunca recalculado com a config atual),
  // qualquer que seja o tamanho do período. Antes, período de 1 dia (Hoje/
  // Ontem) usava total_due — o total "ao vivo" que mistura a cota com o
  // tamanho da carteira de 2º/3º contato — inflando o denominador e
  // escondendo o quanto o corretor realmente ultrapassou a própria cota
  // (exatamente a confusão meta×capacidade corrigida neste pente-fino). Um
  // dia em que o corretor nunca abriu a Meta Diária simplesmente não
  // contribui nada ao previsto (não existia meta gerada naquele dia) — nunca
  // inventa um valor usando a cota vigente de hoje, que é exatamente a
  // recalculação retroativa que a regra da Meta Diária proíbe (ver migration
  ...
  for (const row of goals || []) {
    ...
    entry.previstas += row.new_quota || 0;
    if (row.closed_at) {
...
function buildOwnerTeamOverview({ dailyPerf, overview, profiles, currentQuota, walletByBroker }) {
  ...
  const brokers = profiles
    ...
    .map((profile) => {
      ...
      const activeWalletTotal = walletByBroker?.get(profile.id)?.current || 0;
      const total = activeWalletTotal || (entry ? entry.previstas : currentQuota * daysInRange);
      const contatos = done;
