...
async function buildDailyGoalSnapshot(brokerId, today) {
  ...
  // Group counters describe the cadence; progress uses today's full target.
  ...
  const quota = wallet.requiredToday;
...
function buildOwnerTeamOverview({ dailyPerf, overview, profiles, currentQuota, walletByBroker }) {
  ...
  const brokers = profiles
    ...
    .map((profile) => {
      ...
      const total = isToday ? (walletByBroker?.get(profile.id)?.requiredToday ?? currentQuota) : (entry ? entry.previstas : currentQuota * daysInRange);
      const contatos = done;
