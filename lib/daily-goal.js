...
function buildOwnerTeamOverview({ dailyPerf, overview, profiles, currentQuota }) {
  ...
  const brokers = profiles
    ...
    .map((profile) => {
      ...
      const total = entry ? entry.previstas : currentQuota;
      const contatos = done;
      const atendimentos = funnelRow?.service || 0;
      const simulacoes = funnelRow?.simulation || 0;
      const percent = total ? Math.min(100, Math.round((done / total) * 100)) : (done > 0 ? 100 : 0);
  ...
  return {
    ...
    summary: {
      metaPercent: teamTotal ? Math.min(100, Math.round((teamDone / teamTotal) * 100)) : 0,
...
/* ------------------ Widget global "Top 1 do dia" ------------------ */

// Mesma população e o mesmo critério de colocação do ranking diário do
// painel gerencial (buildOwnerTeamOverview): perfis ativos exceto o dono,
// meta do dia = daily_goals.total_due (ou a quota atual configurada quando
// o corretor ainda não abriu a própria Meta Diária hoje — mesma regra da
// correção anterior), progresso = tentativas registradas hoje, desempate
// idêntico (percentual > concluídas > nome). Não é um ranking novo: é o
// mesmo cálculo, só que consultado direto (sem o período/funil completo do
// painel gerencial, que essa função não usa) para poder ser chamado por
// qualquer usuário autenticado — o widget precisa aparecer para o corretor
// em qualquer tela, e getDailyGoalPerformance é restrito a admin/gestor.
// Resposta mínima (Top 1 + posição de quem perguntou), nunca o
// detalhamento da equipe inteira.
export async function getDailyGoalTopRanking(auth) {
  if (!auth?.ok || !auth?.profile?.id) throw new Error("Usuário sem perfil ativo.");
  if (!hasSupabaseAdminConfig) return null;

  const today = getTodayInSaoPaulo();
  const [profiles, currentQuota, goalsResult, attemptsResult] = await Promise.all([
    listAdminProfiles(),
    getCurrentDailyGoalQuota(),
    db().from("daily_goals").select("broker_id, total_due").eq("goal_date", today),
    db().from("daily_goal_attempts").select("broker_id").eq("goal_date", today)
  ]);
  if (goalsResult.error) throw goalsResult.error;
  if (attemptsResult.error) throw attemptsResult.error;

  const totalByBroker = new Map((goalsResult.data || []).map((row) => [row.broker_id, row.total_due]));
  const doneByBroker = new Map();
  for (const row of attemptsResult.data || []) {
    doneByBroker.set(row.broker_id, (doneByBroker.get(row.broker_id) || 0) + 1);
  }

  const ranking = profiles
    .filter((profile) => profile.id && profile.status !== "inactive" && !isOwnerAdminEmail(profile.email))
    .map((profile) => {
      const done = doneByBroker.get(profile.id) || 0;
      const total = totalByBroker.has(profile.id) ? totalByBroker.get(profile.id) : currentQuota;
      const percent = total ? Math.min(100, Math.round((done / total) * 100)) : (done > 0 ? 100 : 0);
      return { brokerId: profile.id, name: profile.name, photoUrl: profile.photoUrl || "", done, percent };
    })
    .sort((a, b) => b.percent - a.percent || b.done - a.done || a.name.localeCompare(b.name, "pt-BR"));

  if (!ranking.length) return null;

  const myIndex = ranking.findIndex((row) => row.brokerId === auth.profile.id);
  const top1 = ranking[0];

  return {
    top1: { brokerId: top1.brokerId, name: top1.name, photoUrl: top1.photoUrl },
