...
import { isGeneralAdminAuth, listAdminProfiles } from "./admin-profiles";
...
const PERFORMANCE_PERIODS = { TODAY: "today", LAST_7_DAYS: "last7", LAST_30_DAYS: "last30", CUSTOM: "custom" };
...
function resolveDailyGoalRange(params = {}) {
  ...
  let startDate = today;
  let endDate = today;
  if (period === PERFORMANCE_PERIODS.LAST_7_DAYS) startDate = addDaysToPlainDate(today, -6);
  ...
  return {
    ...
    label: period === PERFORMANCE_PERIODS.TODAY ? "Hoje" : period === PERFORMANCE_PERIODS.LAST_7_DAYS ? "Últimos 7 dias" : period === PERFORMANCE_PERIODS.LAST_30_DAYS ? "Últimos 30 dias" : `${formatPlainDateBR(startDate)} a ${formatPlainDateBR(endDate)}`
  };
}
