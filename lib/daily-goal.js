...
async function buildDailyGoalSnapshot(brokerId, today) {
  ...
  const pendingSecondRounds = secondCohort.filter((round) => round.status === "active" && round.attempt_count === 1);
  const pendingThirdRounds = thirdCohort.filter((round) => round.status === "active" && round.attempt_count === 2);
