...
export async function registerDailyGoalAttempt(roundId, customText, auth) {
  ...
  if (config.allowPersonalization && trimmedCustom && trimmedCustom !== finalText) {
    ...
    isPersonalized = true;
    // A edição vira o novo padrão DESSE corretor para esta mensagem (item
    // pedido: não precisar editar de novo a cada envio) — guarda como
    // template reutilizável, não o texto literal deste cliente específico.
    await saveBrokerMessageOverride(brokerId, messageKey, extractTemplateFromEditedText(trimmedCustom, vars));
  }
