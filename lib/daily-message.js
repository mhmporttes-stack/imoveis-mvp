...
export async function listDailyMessageCards({ search = "", type = "all", status = "all" } = {}, auth) {
  ...
  if (type === "biblical" || type === "reflection") query = query.eq("type", type);
...
export async function updateDailyMessageCard(id, payload, auth) {
  ...
  if (payload.type !== undefined) updates.type = payload.type === "reflection" ? "reflection" : "biblical";
...
function normalizeCardPayload(payload) {
  ...
  const type = payload?.type === "reflection" ? "reflection" : "biblical";
