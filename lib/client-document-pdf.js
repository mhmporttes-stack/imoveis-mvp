...
const INCOME_LABELS = { registered_employment: "CLT / registrado", income_tax_declarant: "Declarante de IR", self_employed_unregistered: "Autônomo" };
const STATUS_LABELS = { conforme: "Conforme", pendencia: "Pendência", ilegivel: "Ilegível", ausente: "Ausente", divergencia: "Divergência", precisa_confirmacao: "A confirmar" };
...
async function getChecklistSummary(clientId) {
  const { data } = await db()
    ...
    .select("person_label, document_type, status")
    .eq("client_id", clientId)
    .order("person_label", { ascending: true });
