...
export async function createCalendarActivity(payload, auth) {
  ...
  let responsibleUserId = payload.responsibleUserId || null;
  if (isBrokerProfile(auth?.profile) && !isGeneralAdminAuth(auth)) responsibleUserId = auth.profile.id;
  if (responsibleUserId) assertCanAccessResponsibleUser(auth, responsibleUserId);
  else if (!isGeneralAdminAuth(auth)) throw new Error("Informe o responsável pela atividade.");
...
async function getActivity(id, auth) {
  ...
  if (data.responsible_user_id) assertCanAccessResponsibleUser(auth, data.responsible_user_id);
  else if (!isGeneralAdminAuth(auth)) throw new Error("Acesso negado.");
  return rowToActivity(data);
}
