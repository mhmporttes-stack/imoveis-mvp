...
export async function listCalendarClientOptions(auth) {
  let query = getSupabaseAdminClient().from("simulation_registrations").select("id, full_name, client_code, responsible_user_id").order("full_name");
  query = applyResponsibleUserScope(query, auth);
  const { data, error } = await query;
  if (error) throw error;
  return (data || []).map((row) => ({ id: row.id, name: row.full_name || "Cliente sem nome", code: row.client_code || "", responsibleUserId: row.responsible_user_id || "" }));
}
...
export async function createCalendarActivity(payload, auth) {
  ...
  if (activity.clientId) await logActivityEvent(activity.clientId, "activity_scheduled", auth, { title: activity.title, scheduledAt: activity.scheduledActivityAt, activityType: activity.activityType });
  return activity;
}
...
export async function completeCalendarActivity(id, auth) {
  ...
  if (error) throw error;
  const activity = rowToActivity(data);
  if (activity.clientId) await logActivityEvent(activity.clientId, "activity_completed", auth, { title: activity.title });
  return activity;
}
...
export async function deleteCalendarActivity(id, auth) {
  ...
  if (error) throw error;
  if (current.clientId) await logActivityEvent(current.clientId, "activity_deleted", auth, { title: current.title });
  return { deleted: true };
}

// Ponto único de escrita da linha do tempo pra este arquivo — best-effort,
// nunca pode impedir a ação real (agendar/concluir/excluir/reagendar) de
// acontecer, só registra o rastro dela.
async function logActivityEvent(clientId, eventType, auth, details) {
  try {
    const { logClientJourneyEvent, resolveActorSnapshot } = await import("./client-journey");
    await logClientJourneyEvent({ clientId, eventType, actor: resolveActorSnapshot(auth), details });
  } catch (error) {
    console.warn(`Falha ao registrar "${eventType}" na linha do tempo do cliente:`, error?.message || error);
  }
}
...
export async function rescheduleCalendarActivity(id, scheduledAtValue, noteValue, auth) {
  ...
  const previous = rowToActivity(previousRow);
  const next = rowToActivity(nextRow);
  if (next.clientId) await logActivityEvent(next.clientId, "activity_rescheduled", auth, { title: next.title, fromAt: previous.scheduledActivityAt, toAt: next.scheduledActivityAt });
