...
export async function listWhatsappMessageTemplates() {
  ...
  url.searchParams.set("fields", "id,name,status,category,language,components,quality_score");
...
async function syncBroadcastMessageStatuses(statusEvents) {
  ...
  for (const event of statusEvents) {
    ...
    if (!nextStatus || !event.message_id) continue;

    const { data: current, error: readError } = await supabase
      .from("whatsapp_broadcast_messages")
      .select("id, broadcast_id, status")
      .eq("whatsapp_message_id", event.message_id)
      .maybeSingle();
    if (readError || !current) continue; // mensagem de fora de uma campanha de Disparo — ignora
    ...
    if (!updateError) broadcastIdsTouched.add(current.broadcast_id);
  }

  for (const broadcastId of broadcastIdsTouched) await recomputeBroadcastCounters(broadcastId);
}

async function recomputeBroadcastCounters(broadcastId) {
  const supabase = getSupabaseAdminClient();
  const { data, error } = await supabase.from("whatsapp_broadcast_messages").select("status").eq("broadcast_id", broadcastId);
