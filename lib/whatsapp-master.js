...
export async function createWhatsappMessageTemplate({ name, category = "UTILITY", languageCode = "pt_BR", bodyText, bodyExample = [] }) {
  const token = process.env.WHATSAPP_ACCESS_TOKEN || "";
  ...
  const component = { type: "BODY", text: cleanBody };
  ...
  const response = await fetch(`https://graph.facebook.com/${version}/${encodeURIComponent(businessAccountId)}/message_templates`, {
    ...
    body: JSON.stringify({ name: cleanName, language: languageCode, category, components: [component] }),
    cache: "no-store",
  ...
  return { id: payload?.id || "", status: payload?.status || "PENDING", category: payload?.category || category };
}
