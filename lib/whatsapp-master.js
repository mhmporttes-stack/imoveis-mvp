...
import { canonicalWhatsappPhone, digitsOnly, toWhatsAppDigits } from "./phone-utils";
import { findConversationByPhone, findLatestRegistrationIdsByPhones, findRecentFormRegistration } from "./client-phone-lookup";
import { isFormCompletionMessage, shouldSuppressKeywordReply } from "./whatsapp-form-completion.mjs";
import { isHumanAttendingPhone } from "./whatsapp-attendance";
import { getSupabaseAdminClient } from "./supabase";
import {
  buildAutomationResponseText,
  findResponsibleBrokerProfileByPhone,
...
// allowRawRecipient: usado pelo CHAT para RESPONDER a quem escreveu — o
// número vem da própria Meta (wa_id), então vale mesmo quando não passa na
// validação de celular brasileiro (ex.: sem o nono dígito, número de fora).
export async function sendWhatsappTextMessage({ to, text, allowRawRecipient = false }) {
  const token = process.env.WHATSAPP_ACCESS_TOKEN || "";
  const phoneNumberId = process.env.WHATSAPP_PHONE_NUMBER_ID || "";
  const recipient = toWhatsAppDigits(to) || (allowRawRecipient ? digitsOnly(to) : "");
...
// Mensagem já no formato da Cloud API ({type:"text"|"interactive", ...}) —
// usada pelos Fluxos (botões de resposta, lista, botão de link). Mesmas regras
// da mensagem de texto livre: só dentro da janela de 24h e nunca expõe o token.
export async function sendWhatsappMessagePayload({ to, message, allowRawRecipient = true }) {
  const token = process.env.WHATSAPP_ACCESS_TOKEN || "";
  const phoneNumberId = process.env.WHATSAPP_PHONE_NUMBER_ID || "";
  const recipient = toWhatsAppDigits(to) || (allowRawRecipient ? digitsOnly(to) : "");
  ...
  if (!token || !phoneNumberId) throw new Error("Credenciais da Meta Cloud API não configuradas.");
  if (!recipient) throw new Error("WhatsApp do destinatário inválido.");
  if (!message || !["text", "interactive", "image", "audio", "document"].includes(message.type)) throw new Error("Mensagem inválida.");
...
async function processAutomationReply(event) {
  // Atendimento humano ativo (uma pessoa respondeu este cliente pelo Chat há menos
  // de 30 min): a resposta automática por palavra-chave não interfere na conversa.
  if (await isHumanAttendingPhone(event.sender_phone)) {
    console.info(JSON.stringify({ source: "whatsapp-automation-reply", event: "skipped_human_attending", messageId: event.message_id }));
    return;
  }

  // Cliente que ACABOU de preencher o formulário (mensagem do botão "Receber minha simulação"
  // ou cadastro recente por formulário/link) já tem corretor: a resposta automática não interfere.
  const isFormMessage = isFormCompletionMessage(event.message_text);
  const recentForm = isFormMessage ? null : await findRecentFormRegistration(event.sender_phone);
  if (shouldSuppressKeywordReply({ isFormMessage, recentForm: Boolean(recentForm) })) {
    console.info(JSON.stringify({ source: "whatsapp-automation-reply", event: "skipped_post_form_contact", messageId: event.message_id, byMessage: isFormMessage }));
    return;
  }
  ...
  if (rule.forwardToRoleta) {
    const conversation = await findConversationByPhone(event.sender_phone, "id");
    const result = await materializeClientFromAutomationReply({
  ...
  // Já é cliente (ex.: repetiu o texto do anúncio): usa o link do corretor responsável, não o genérico.
  if (!brokerProfile) brokerProfile = await findResponsibleBrokerProfileByPhone(event.sender_phone);

  const responseText = buildAutomationResponseText(rule.responseMessage, { brokerProfile });
  const sent = await sendWhatsappTextMessage({ to: event.sender_phone, text: responseText, allowRawRecipient: true });
  await incrementAutomationReplyTriggerCount(rule.id);

  // Aparece no CHAT como mensagem de "automação" (best-effort — nunca derruba
  // a automação em si).
  try {
    ...
    await recordOutboundAutomationMessage({
      ...
      metadata: { kind: "keyword_reply" }
    });
  } catch (chatError) {
    console.warn("Falha ao registrar a resposta automática no CHAT:", chatError?.message || chatError);
  }
}
...
// callbackData: texto opaco (até 512 caracteres) que a Meta devolve nos webhooks de
// status desta mensagem (biz_opaque_callback_data) — o Disparo usa o ID da própria
// linha para reconciliar envios interrompidos sem reenviar.
//
// Erros lançados aqui carregam `metaRejected = true` quando a Meta RESPONDEU com
// erro, ou `notSent = true` quando falhou antes de chamar a Meta (validação local) —
// nos dois casos a mensagem certamente NÃO foi aceita. Sem essas marcas (timeout, rede, resposta
// sem ID) o resultado é DESCONHECIDO: a Meta pode ter aceitado — quem chama nunca
// deve reenviar automaticamente.
function notSentError(message) {
  const error = new Error(message);
  error.notSent = true;
  return error;
}

export async function sendWhatsappTemplateMessage({ to, templateName, languageCode = "pt_BR", bodyParameters = [], buttonUrlParameter = "", callbackData = "" }) {
  const token = process.env.WHATSAPP_ACCESS_TOKEN || "";
  const phoneNumberId = process.env.WHATSAPP_PHONE_NUMBER_ID || "";
  const recipient = toWhatsAppDigits(to);
  const cleanTemplateName = String(templateName || "").trim();

  // Falhas ANTES de qualquer chamada à Meta: a mensagem certamente não foi enviada (notSent).
  if (!token || !phoneNumberId) throw notSentError("Credenciais da Meta Cloud API não configuradas.");
  if (!recipient) throw notSentError("WhatsApp do destinatário inválido.");
  if (!cleanTemplateName) throw notSentError("Modelo de lembrete do WhatsApp não configurado.");
  ...
  const response = await fetch(`https://graph.facebook.com/${version}/${encodeURIComponent(phoneNumberId)}/messages`, {
    ...
    body: JSON.stringify({
      ...
      type: "template",
      template,
      ...(callbackData ? { biz_opaque_callback_data: String(callbackData).slice(0, 512) } : {})
    }),
    cache: "no-store",
    signal: AbortSignal.timeout(12000)
  });
  ...
  if (!response.ok || payload?.error) {
    const rejection = new Error(sanitizeMetaError(payload?.error));
    rejection.metaRejected = true;
    throw rejection;
  }
...
// Usado antes de criar um modelo novo, para não tentar recriar um que já
// existe — a Meta rejeita nome duplicado. Também é a base da sincronização
// do Disparo (item 9 do pedido):
...
export async function processWhatsappWebhook(payload) {
  ...
  const clientByPhone = await findLatestRegistrationIdsByPhones(events.map((event) => event.sender_phone));
  ...
  const { data, error } = await supabase
    ...
    .select("id, event_type, direction, sender_phone, contact_name, message_text, message_id, message_type, raw_payload");
  if (error) throw error;

  // CHAT (WhatsApp Master > Chat): projeta mensagens recebidas em
  // conversa/mensagem e aplica os status de entrega das mensagens enviadas
  // pelo CRM. Idempotente (chaveado pelo ID da Meta) e best-effort: uma falha
  // aqui não pode impedir a automação/sincronizações abaixo nem fazer a Meta
  // reenviar o webhook inteiro — o evento bruto já está em whatsapp_master_events.
  try {
    const { projectChatFromEvents } = await import("./whatsapp-chat");
    await projectChatFromEvents(events, clientByPhone);
  } catch (chatError) {
    console.error("Falha ao projetar o webhook no CHAT do WhatsApp:", chatError?.message || chatError);
  }

  // Lead PATROCINADO (Click to WhatsApp) que ainda não é cliente: entra na roleta existente e a conversa
  // vai para o mesmo corretor. Roda antes das automações (o cliente já existe quando elas rodam) e nunca
  // derruba o webhook (a rede de segurança do cron reconcilia se falhar).
  try {
    const { processSponsoredLeads } = await import("./whatsapp-sponsored-lead");
    await processSponsoredLeads(data || []);
  } catch (sponsoredError) {
    console.error("Falha ao processar o lead patrocinado:", sponsoredError?.message || sponsoredError);
  }

  // Contato direto (sem anúncio) que ainda não é cliente: cadastro automático pela roleta, mesmo caminho do
  // patrocinado. Roda antes das automações (o cliente já existe quando elas rodam) e nunca derruba o webhook
  // (a rede de segurança do cron reconcilia se falhar).
  try {
    const { processOrganicLeads } = await import("./whatsapp-sponsored-lead");
    await processOrganicLeads(data || []);
  } catch (organicError) {
    console.error("Falha ao cadastrar o contato direto do WhatsApp:", organicError?.message || organicError);
  }

  // Áudio recebido: o servidor baixa da Meta e guarda em storage privado (o Chat toca dali).
  try {
    const { downloadInboundAudiosForEvents } = await import("./whatsapp-media");
    await downloadInboundAudiosForEvents(data || []);
  } catch (mediaError) {
    console.error("Falha ao baixar a mídia recebida do WhatsApp:", mediaError?.message || mediaError);
  }
  ...
  if (newInboundMessages.length) {
    ...
    // Fluxos (WhatsApp Master > Fluxos) têm prioridade: se o cliente está no
    // meio de um fluxo, ou a mensagem dispara um fluxo ativo, ele cuida da
    // resposta e a resposta automática por palavra-chave (abaixo) não roda.
    // Sequencial (não em paralelo) para não disputar a mesma sessão quando o
    // mesmo cliente manda várias mensagens num webhook só.
    for (const event of newInboundMessages) {
      let handledByFlow = false;
      try {
        const { processFlowInbound } = await import("./whatsapp-flows");
        handledByFlow = await processFlowInbound(event);
      } catch (flowError) {
        console.warn("Falha ao processar Fluxo do WhatsApp:", flowError?.message || flowError);
      }
      if (handledByFlow) continue;
      await processAutomationReply(event).catch((automationError) => {
...
async function syncBroadcastMessageStatuses(statusEvents) {
  ...
  await reconcileInterruptedBroadcastSends(supabase, relevantEvents);
  ...
  for (const event of relevantEvents) {
    ...
    // A Meta informa em cada status se a mensagem é cobrada e a categoria — base dos "Gastos" do Disparo.
    const pricing = event.raw_payload?.status?.pricing;
    if (pricing && typeof pricing.billable === "boolean") {
      update.billable = pricing.billable;
      if (pricing.category) update.pricing_category = String(pricing.category).toLowerCase();
    }
...
// Envio interrompido no meio (processo caiu depois de chamar a Meta e antes de gravar
// o ID): a linha ficou 'delivery_unknown' — e NÃO foi reenviada. Se a Meta tiver
// aceitado, o webhook de status devolve o biz_opaque_callback_data ("bcm:<id da
// linha>") que o Disparo enviou; aqui a linha ganha o ID da Meta e volta a 'sent',
// e o fluxo normal abaixo avança para entregue/lida.
async function reconcileInterruptedBroadcastSends(supabase, statusEvents) {
  const byRowId = new Map();
  for (const event of statusEvents) {
    const callback = String(event.raw_payload?.status?.biz_opaque_callback_data || "");
    if (!callback.startsWith("bcm:")) continue;
    if (!byRowId.has(callback.slice(4))) byRowId.set(callback.slice(4), event);
  }
  if (!byRowId.size) return;

  const { data: rows, error } = await supabase
    .from("whatsapp_broadcast_messages")
    .select("id, status, error_code, whatsapp_message_id, audit")
    .in("id", [...byRowId.keys()])
    .is("whatsapp_message_id", null);
  if (error || !rows?.length) return;

  for (const row of rows) {
    const interrupted = row.status === "processing" || (row.status === "failed" && row.error_code === "delivery_unknown");
    if (!interrupted) continue;
    const event = byRowId.get(row.id);
    const at = event.event_at || new Date().toISOString();
    await supabase.from("whatsapp_broadcast_messages").update({
      ...
      error_message: null,
      claim_token: null,
      audit: [...(Array.isArray(row.audit) ? row.audit : []), { at: new Date().toISOString(), event: "reconciled_by_webhook", messageId: event.message_id }]
    }).eq("id", row.id).is("whatsapp_message_id", null);
  }
}
...
// Telefone de quem escreve, no formato CANÔNICO (celular brasileiro sempre com o
// 9º dígito): a Meta às vezes entrega o wa_id sem o 9 — sem isto o mesmo contato
// abriria outra conversa e não acharia o cliente já cadastrado.
...
export function normalizeGraphVersion(value) {
