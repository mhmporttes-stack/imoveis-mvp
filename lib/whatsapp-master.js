...
// TEMP-TESTE (remover junto com app/api/admin/tmp-critical-tests): números com DDD 00 não
// existem no Brasil — nunca chamam a Meta; devolve um ID falso e registra o envio simulado.
...
// erro (a mensagem certamente NÃO foi aceita). Sem essa marca (timeout, rede, resposta
...
export async function sendWhatsappTemplateMessage({ to, templateName, languageCode = "pt_BR", bodyParameters = [], buttonUrlParameter = "", callbackData = "" }) {
  ...
  const dryRunTemplate = dryRunForFictionalRecipient("template", recipient); // TEMP-TESTE
  if (dryRunTemplate) return dryRunTemplate; // TEMP-TESTE
