...
// TEMP-TESTE (remover junto com app/api/admin/tmp-chat-tests): números com DDD 00 não existem no Brasil —
// nunca chamam a Meta; devolve um ID falso e registra o envio simulado.
function dryRunForFictionalRecipient(kind, recipient, text = "") {
  if (!/^5500/.test(String(recipient || ""))) return null;
  const messageId = `wamid.DRYRUN.${randomUUID()}`;
  (globalThis.__waDryRun ||= []).push({ kind, recipient, messageId, text: String(text || "").slice(0, 200) });
...
export async function sendWhatsappTextMessage({ to, text, allowRawRecipient = false }) {
  ...
  const dryRunText = dryRunForFictionalRecipient("text", recipient, body); // TEMP-TESTE
  if (dryRunText) return dryRunText; // TEMP-TESTE
...
export async function sendWhatsappMessagePayload({ to, message, allowRawRecipient = true }) {
  ...
  const dryRunPayload = dryRunForFictionalRecipient(message.type, recipient); // TEMP-TESTE
  if (dryRunPayload) return dryRunPayload; // TEMP-TESTE
