...
const CLIENT_COLUMNS = "id, full_name, client_code, status, responsible_user_id, last_whatsapp_contact_at, created_at, scheduled_activity_at, scheduled_activity_completed_at";
const DAY_MS = 24 * 60 * 60 * 1000;
const REMAINING_CLIENTS_LIMIT = 30;
...
// `withClients: true` (detalhe do dono) também devolve QUAIS clientes ainda faltam
// (mais antigos primeiro, até 30) com há quantos dias estão sem contato.
export async function getDailyGoalPendingProgress(brokerId, day, { attemptClientIds = null, source = "lazy", createIfMissing = true, withClients = false } = {}) {
  let frozen;
  if (createIfMissing) {
  ...
  if (!ids.length) return { total: 0, done: 0, remaining: 0, ...base, ...(withClients ? { clients: [], moreClients: 0 } : {}) };
  ...
  const { remainingIds, ...counts } = evaluateDailyGoalPending({ brokerId, frozenIds: ids, clientsById, dayStartMs, nowMs, attemptClientIds: attempted });
  const result = { ...counts, ...base };
  if (withClients) {
    const referenceOf = (client) => Date.parse(client.lastWhatsappContactAt || client.createdAt || "") || nowMs;
    const sorted = remainingIds.map((id) => clientsById.get(id)).filter(Boolean).sort((a, b) => referenceOf(a) - referenceOf(b));
    result.clients = sorted.slice(0, REMAINING_CLIENTS_LIMIT).map((client) => ({
      id: client.id,
      fullName: client.fullName || "Cliente",
      clientCode: client.clientCode || "",
      daysWithoutContact: Math.max(0, Math.floor((nowMs - referenceOf(client)) / DAY_MS))
    }));
    result.moreClients = Math.max(0, sorted.length - REMAINING_CLIENTS_LIMIT);
  }
  return result;
}
