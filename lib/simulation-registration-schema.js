...
export const PRIMARY_INCOME_OPTIONS = [
  { value: "registered_employment", label: "Trabalho com carteira assinada (holerite)" },
  { value: "income_tax_declarant", label: "Declaro Imposto de Renda" },
  { value: "self_employed_unregistered", label: "Sou autônomo(a), sem registro e sem declaração de Imposto de Renda" }
];

export const SECONDARY_INCOME_OPTIONS = [
  { value: "registered_employment", label: "Trabalho com carteira assinada (holerite)" },
  { value: "income_tax_declarant", label: "Declara Imposto de Renda" },
  { value: "self_employed_unregistered", label: "É autônomo(a), sem registro e sem declaração de Imposto de Renda" }
];
...
export function buildRegistrationSteps(values = {}) {
  ...
  const steps = [
    ...
    },
    {
      ...
      inputMode: "text",
      placeholder: "Ex.: vendedor, professora, autônomo"
    },
    {
  ...
  if (isJoint) {
    steps.push(
      ...
      {
        id: "secondaryProfession",
        kind: "text",
        title: "Qual é a profissão da segunda pessoa?",
        inputMode: "text",
        placeholder: "Ex.: motorista, enfermeira, autônoma"
      },
      {
...
const nameSchema = sanitizedString(160).refine(
  ...
  (value) => countNameParts(value) >= 2,
  "Informe pelo menos nome e sobrenome."
);

const professionSchema = sanitizedString(120).refine(
  (value) => /[A-Za-zÀ-ÿ]/.test(value),
  "Informe uma profissão válida."
);
...
export const simulationRegistrationSchema = z
  .object({
    ...
    primaryProfession: professionSchema,
    primaryMonthlyIncome: moneySchema,
    secondaryIncomeType: optionalEnum(INCOME_TYPE_VALUES),
    secondaryProfession: optionalTextSchema,
    secondaryMonthlyIncome: optionalMoneySchema,
  ...
  .superRefine((data, context) => {
    ...
    if (!data.secondaryProfession || !/[A-Za-zÀ-ÿ]/.test(data.secondaryProfession)) {
      context.addIssue({ code: "custom", path: ["secondaryProfession"], message: "Informe a profissão da segunda pessoa." });
    }
  ...
  .transform((data) => ({
    ...
    secondaryProfession: data.simulationType === "joint" ? sanitizeText(data.secondaryProfession) : null,
...
export function validateStepValue(step, values) {
  ...
  if (step.kind === "currency") {
    const parsed = parseCurrencyNumber(value);
    return Number.isFinite(parsed) && parsed >= 0 ? "" : "Informe um valor válido.";
  }

  const text = sanitizeText(value);
  if (!text) return "Campo obrigatório.";
  if (!/[A-Za-zÀ-ÿ]/.test(text)) return "Informe uma resposta válida.";
  if (step.id === "fullName" && countNameParts(text) < 2) return "Informe pelo menos nome e sobrenome.";
  return "";
}
...
function countNameParts(value) {
  return sanitizeText(value)
    .split(" ")
    .filter((part) => /[A-Za-zÀ-ÿ]{2,}/.test(part))
    .length;
}
