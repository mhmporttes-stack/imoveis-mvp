...
export function formatPersonName(value) {
  const lowerCaseParticles = new Set(["da", "das", "de", "di", "do", "dos", "e"]);
  return sanitizeText(value)
    .toLocaleLowerCase("pt-BR")
    .split(" ")
    .map((part, index) => {
      if (index > 0 && lowerCaseParticles.has(part)) return part;
      return part
        .split("-")
        .map((segment) => segment ? `${segment.charAt(0).toLocaleUpperCase("pt-BR")}${segment.slice(1)}` : segment)
        .join("-");
    })
    .join(" ");
}
