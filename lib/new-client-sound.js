...
// Preferência por dispositivo/navegador E por usuário — a chave inclui o id
// de quem está logado, para que num computador compartilhado (ex.: "PC da
// imobiliária" usado por vários corretores) cada login tenha o próprio
// Ativado/Desativado, sem um usuário ligar o alerta e isso valer para
// qualquer outra pessoa que logar depois nesse mesmo navegador. Continua
// sendo só localStorage (sem coluna no Supabase) — só passou a ser
// namespaced por userId dentro dele.
const STORAGE_KEY_PREFIX = "crm_new_client_sound_enabled:";

function storageKey(userId) {
  return `${STORAGE_KEY_PREFIX}${userId || "anon"}`;
}

export function isNewClientSoundEnabled(userId) {
...
export function setNewClientSoundEnabled(userId, value) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(storageKey(userId), value ? "true" : "false");
  } catch {
