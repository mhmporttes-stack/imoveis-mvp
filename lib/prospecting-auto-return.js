...
export async function autoReturnStaleProspectingContacts(now = new Date()) {
  ...
  for (const contact of stale) {
    ...
    const { error: clientError } = await db.from("simulation_registrations").update({ responsible_user_id: null, last_status_change_at: now.toISOString() }).eq("id", contact.registration_id).eq("status", CLIENT_STATUS.AWAITING_RETURN);
    if (clientError) throw clientError;
    const { error: historyError } = await db.from("prospecting_history").insert({ contact_id: contact.id, registration_id: contact.registration_id, user_id: contact.assigned_user_id, event_type: "auto_returned", details: { availableAfter, reason: "attempt_timeout_2_days" } });
