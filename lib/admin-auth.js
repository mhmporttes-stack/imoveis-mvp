...
import { getSupabasePublicClient, hasSupabasePublicConfig, supabaseDiag } from "./supabase";
import {
  ADMIN_ROLE,
  ADMIN_USER_STATUS,
...
export async function verifyAdminAccessToken(accessToken) {
  if (!accessToken || !hasSupabasePublicConfig) {
    return {
      ...
      diag: { reason: "missing-token-or-config", hasAccessToken: Boolean(accessToken), ...supabaseDiag }
    };
  }
...
async function buildAuthorizedAdminResult(user) {
  ...
  if (!profile) {
    if (!isAuthorizedAdminEmail(user.email)) {
      return { ok: false, status: 403, error: "Acesso nao autorizado.", user, diag: { reason: "email-not-authorized" } };
    }
