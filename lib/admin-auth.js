...
export async function requireAdminApi(request) {
  ...
  if (effectiveResult.ok && effectiveResult.auditMode && !["GET", "HEAD", "OPTIONS"].includes(request.method)) {
    return {
      ...effectiveResult,
      ok: false,
      status: 403,
      error: "Modo somente leitura. Volte para seu perfil para realizar alterações."
    };
  }
