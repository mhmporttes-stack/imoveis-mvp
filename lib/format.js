...
export function propertyCardFeatures(property = {}, limit = 4) {
  const explicitFeatures = Array.isArray(property.features)
    ? property.features.map(cleanFeature).filter(Boolean)
    : [];

  const fallbackFeatures = [
    normalizeBedrooms(property.bedrooms),
    normalizeArea(property.area),
    property.installmentEntry ? "Entrada facilitada" : "",
    hasFinancingText(property) ? "Aceita financiamento" : "",
    property.type === "loteamento" ? "Pronto para construir" : ""
  ].filter(Boolean);

  return [...new Set([...explicitFeatures, ...fallbackFeatures])].slice(0, limit);
}
