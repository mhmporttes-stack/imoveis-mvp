...
export async function materializeClientFromAutomationReply({ phone, name, conversationId = null, via = "keyword_reply", details = {} }) {
  ...
  const { data, error } = await db().rpc("whatsapp_get_or_create_roulette_client", {
    ...
    // Conversa vinculada e atribuída ao MESMO corretor + histórico da roleta (mesma transação do cliente).
    p_conversation_id: conversationId,
    p_history_details: { source: "whatsapp", via, ...details }
  });
  if (error) throw error;
...
// Contato que já é cliente: o link de simulação da resposta automática é o do corretor RESPONSÁVEL por ele (não o
// genérico), para o cadastro que ele fizer cair com o corretor que já o atende.
export async function findResponsibleBrokerProfileByPhone(phone) {
  try {
    const { findLatestRegistrationIdsByPhones } = await import("./client-phone-lookup");
    const matches = await findLatestRegistrationIdsByPhones([phone]);
    const registrationId = matches.get(phone);
    if (!registrationId) return null;
    const { data } = await getSupabaseAdminClient().from("simulation_registrations").select("responsible_user_id").eq("id", registrationId).maybeSingle();
    return data?.responsible_user_id ? await getAdminProfileById(data.responsible_user_id) : null;
  } catch (error) {
    console.warn("Falha ao buscar o corretor responsável pelo telefone:", error?.message || error);
    return null;
  }
}
