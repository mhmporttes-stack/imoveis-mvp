...
// A palavra-chave precisa aparecer como palavra inteira: "sim" bate com "sim,
// quero!" mas NÃO com "simulação" nem "assim" (a mensagem do botão "Receber
// minha simulação" do formulário caía na regra "sim" por engano).
function containsWholeWord(message, keyword) {
...
export async function matchAutomationReply(messageText) {
  ...
  for (const row of data || []) {
    ...
    if (normalizedKeyword && containsWholeWord(normalizedMessage, normalizedKeyword)) return rowToRule(row);
  }
  return null;
}
