...
export function getOwnResourceValue(simulation = {}) {
  return roundMoney(
    normalizeMoneyValue(simulation.downPaymentValue) +
    normalizeMoneyValue(simulation.fgtsValue)
  );
}

export function getSimulationListSummary(simulation = {}) {
  const ownResource = getOwnResourceValue(simulation);
...
export function extractSimulationPhone(simulation = {}) {
  const note = String(simulation.internalNote || "");
