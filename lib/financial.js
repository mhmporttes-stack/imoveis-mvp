...
import { canUseProfileDatabaseScope, isGeneralAdminAuth, isManagerProfile } from "./admin-profiles";
...
export async function listFinancialSales(auth = null) {
  ...
  let query = supabase
    ...
    .select(`
      *,
      client:simulation_registrations!inner(*),
...
export async function ensureFinancialSaleForRegistration(registration, adminEmail = "", saleDate = "") {
...
export async function updateFinancialSale(id, payload = {}, adminEmail = "", auth = null) {
  ...
  const record = {
    ...
    invoice_issued: payload.invoiceIssued !== undefined ? Boolean(payload.invoiceIssued) : current.invoiceIssued,
...
function applyFinancialClientScope(query, auth) {
  if (!auth || isGeneralAdminAuth(auth) || isManagerProfile(auth.profile)) return query;
