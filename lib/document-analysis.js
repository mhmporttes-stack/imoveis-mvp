...
export async function analyzeClientDocumentBatch({ documents, clientContext }) {
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      // mudança necessária.
      system: [{ type: "text", text: FIXED_SYSTEM_INSTRUCTIONS, cache_control: { type: "ephemeral" } }],
...
function buildClientContextText(clientContext, fileCount) {
  ...
  if (clientContext?.fullName) lines.push(`Nome do titular no cadastro do CRM: ${clientContext.fullName} — use isso só para te ajudar a identificar quem é o titular nos documentos, nunca para decidir o que está faltando.`);
  return lines.join(" ");
}
...
5) VALIDADE/VENCIMENTO: para comprovante_residencia, extrato_bancario, fatura_cartao e holerite — documentos com validade curta — marque "expired": true se a data do documento (emissão/competência/vencimento) for claramente antiga demais pra operação (comprovante de residência ou extrato/fatura com mais de ~3 meses, holerite de competência muito antiga). Para os demais tipos de documento (RG, CNH, certidões etc.), sempre "expired": false — eles não têm essa validade curta. Não confunda com "pendencia": um documento pode estar "conforme" e ainda assim "expired": true (é legível e válido como documento, só desatualizado pra esta operação).
        ...
        items: {
          ...
          items: {
            ...
            properties: {
              ...
              expired: { type: "boolean", description: "true só para comprovante_residencia/extrato_bancario/fatura_cartao/holerite claramente desatualizados; sempre false para os demais tipos." },
