...
import { normalizeClientStatus, ACTIVE_CLIENT_STATUS_VALUES as ACTIVE_CLIENT_STATUSES } from "./client-status";
...
export async function updateWhatsappMasterSettings(payload, auth) {
  return saveWhatsappMasterRuntimeSettings(
    { active: payload?.active === true },
    auth?.profile?.id || null
  );
}

export async function saveWhatsappMasterRuntimeSettings(patch, updatedBy = null) {
  const supabase = getSupabaseAdminClient();
  const current = await getWhatsappMasterSettings();
  const settings = normalizeWhatsappMasterSettings({ ...current, ...patch });
  const { data, error } = await supabase
    .from("crm_settings")
    .upsert({
      id: "whatsapp_master",
...
function normalizeWhatsappMasterSettings(value = {}, updatedAt = "") {
  return {
    ...
    connectionStatus: ["not_configured", "disconnected", "connected"].includes(value?.connectionStatus)
      ? value.connectionStatus
      : "not_configured",
    connectionType: "Meta Cloud API",
    phoneNumberId: String(value?.phoneNumberId || value?.connectionId || ""),
    businessAccountId: String(value?.businessAccountId || ""),
    lastConnectedAt: value?.lastConnectedAt || "",
    lastCheckedAt: value?.lastCheckedAt || "",
    lastWebhookAt: value?.lastWebhookAt || "",
