...
export async function getPerformanceOverview(params = {}, auth = null) {
  ...
  const funnel = computeCumulativeFunnel(cohortClientIds, cohortFullHistory, currentStatusByClientId, clientBrokerMap);
...
function computeCumulativeFunnel(cohortClientIds, fullHistory, currentStatusByClientId, clientBrokerMap) {
  ...
  for (const clientId of cohortClientIds) {
    ...
    if (currentIndex >= 0) furthestIndexByClient.set(clientId, currentIndex);
  }
