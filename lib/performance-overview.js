...
export async function getPerformanceOverview(params = {}, auth = null) {
  ...
  // O funil representa clientes que realmente entraram em prospecção no
  // período. Uma tentativa no 2º/3º dia continua sendo prospecção, mas cada
  // cliente entra apenas uma vez, mesmo que tenha várias tentativas.
  const prospectingClientIds = getProspectingClientIds(teamRegistrations, prospectingEvents, range);
...
function getProspectingClientIds(registrations, events, range) {
  const ids = new Set();
  for (const event of events) if (event.client_id) ids.add(event.client_id);
  // Link direto já é uma entrada válida no funil, mesmo sem tentativa manual.
  for (const row of registrations) {
    if (row.direct_broker_link && withinRange(row.created_at, range)) ids.add(row.id);
  }
