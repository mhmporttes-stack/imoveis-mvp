...
const SCORING_STATUS_VALUES = Object.values(CLIENT_STATUS).filter((status) => getScoringKeyForStatus(status));
...
// O widget "Top 1 do dia" roda em toda tela para todo usuário logado e cada
// chamada recalcula o painel inteiro (dezenas de consultas) — com o banco em
// ~97% de CPU isso derrubava o sistema todo. Um cache curto em memória (e uma
// única chamada em andamento compartilhada) mantém o mesmo resultado sem
// multiplicar a carga; 90 s de atraso é irrelevante para um ranking do dia.
const RANKING_CACHE_MS = 90 * 1000;
let rankingOverviewCache = { at: 0, value: null, promise: null };

async function getCachedTodayOverviewForRanking() {
  const now = Date.now();
  if (rankingOverviewCache.value && now - rankingOverviewCache.at < RANKING_CACHE_MS) return rankingOverviewCache.value;
  if (rankingOverviewCache.promise) return rankingOverviewCache.promise;

  const fullVisibilityAuth = { ok: true, user: { email: "" }, profile: { id: "", role: "admin" } };
  rankingOverviewCache.promise = getPerformanceOverview({ period: "today" }, fullVisibilityAuth)
    .then((value) => {
      rankingOverviewCache = { at: Date.now(), value, promise: null };
      return value;
    })
    .catch((error) => {
      rankingOverviewCache.promise = null;
      throw error;
    });
  return rankingOverviewCache.promise;
}
...
async function filterFirstTimeScoringEvents(supabase, history, range) {
  ...
  const simulationClientIds = new Set(scored.filter((row) => getScoringKeyForStatus(row.new_status) === "simulation").map((row) => row.client_id));
  const achievedBefore = new Map();
  const withSimulationValues = new Set();

  // O banco já está no limite de CPU (plano gratuito) e esta função roda a
  // cada leitura do ranking — por isso só busca o estritamente necessário:
  // histórico ANTERIOR apenas dos status que pontuam, e simulações só dos
  // clientes com evento de simulação e só as que têm valor.
  for (const group of chunkArray(chunkArray(clientIds, 150), 5)) {
    const results = await Promise.all(group.map(async (batch) => {
      const simBatch = batch.filter((id) => simulationClientIds.has(id));
      const [prior, simulations] = await Promise.all([
        fetchAllRows((from, to) => supabase
          .from("client_status_history")
          .select("client_id, new_status")
          .in("client_id", batch)
          .in("new_status", SCORING_STATUS_VALUES)
        ...
        simBatch.length
          ? fetchAllRows((from, to) => supabase
            ...
            .in("registration_id", simBatch)
            .or("financing_value.gt.0,subsidy_value.gt.0")
            .order("id", { ascending: true })
            .range(from, to))
          : Promise.resolve([])
      ]);
