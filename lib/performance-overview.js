...
function getProspectingClientIds(registrations, events, range, statusHistory = []) {
  ...
  // Cliente mais antigo que teve alguma mudança de status registrada dentro
  // do período (progrediu no funil agora, mesmo sem evento de prospecção
  // formal nem ter sido criado neste período) também conta.
  for (const row of statusHistory) if (row.client_id) ids.add(row.client_id);
