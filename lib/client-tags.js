...
export async function setClientTags(clientId, tagIds = [], auth = null) {
  ...
  if (added.length || removed.length) {
    try {
      ...
      await logClientJourneyEvents([
        ...added.map((tagId) => ({ clientId, eventType: "tag_added", actor, details: { tagName: nameById.get(tagId) || "" } })),
        ...removed.map((tagId) => ({ clientId, eventType: "tag_removed", actor, details: { tagName: nameById.get(tagId) || "" } }))
      ]);
    } catch (timelineError) {
