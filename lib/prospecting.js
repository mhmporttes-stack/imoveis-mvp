...
export async function listProspectingContacts(auth) {
  ...
  if (!isOwnerAdminEmail(auth?.user?.email) && !isOwnerAdminEmail(auth?.profile?.email)) {
    query = query.in("status", ["available", "recent_attempt"]);
  }
...
export async function assignProspectingContacts(ids, assignedUserId, auth) {
  ...
  for (const batch of chunks(contactIds, 500)) {
    const now = new Date().toISOString();
    const { data, error } = await db().from("prospecting_contacts").update({ assigned_user_id: assignedUserId, last_broker_id: assignedUserId, status: "claimed", available_after: null, updated_at: now }).in("id", batch).select("id, registration_id");
    if (error) throw error;
    const rows = data || [];
    assigned += rows.length;
    const registrationIds = rows.map((row) => row.registration_id).filter(Boolean);
    if (registrationIds.length) {
      const { error: registrationError } = await db().from("simulation_registrations").update({ responsible_user_id: assignedUserId }).in("id", registrationIds);
      if (registrationError) throw registrationError;
    }
    if (rows.length) {
