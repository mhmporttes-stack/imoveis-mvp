...
export async function claimProspectingContact(id, auth) {
  ...
  if (!completion.unlocked) {
    throw new Error(`A Prospecção Extra está bloqueada. Conclua ${completion.required} atividades da carteira (${completion.completed}/${completion.required}) antes de buscar novos contatos.`);
  }
  const now = new Date().toISOString();
