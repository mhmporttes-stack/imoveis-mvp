...
export async function listProspectingContacts(auth, scope = "company") {
  ...
  query = query.or(`status.eq.available,and(status.eq.recent_attempt,available_after.lte.${now})`);
...
export async function listBrokerBasesSummary(auth) {
  ...
  const [profiles, ownerRows] = await Promise.all([
    listAdminProfiles(),
    db().from("prospecting_contacts").select("owner_user_id").not("owner_user_id", "is", null)
  ]);
  if (ownerRows.error) throw ownerRows.error;

  const countByOwner = new Map();
  for (const row of ownerRows.data || []) {
    countByOwner.set(row.owner_user_id, (countByOwner.get(row.owner_user_id) || 0) + 1);
  }

  return profiles
    .filter((profile) => profile.status !== "inactive" && !isOwnerAdminEmail(profile.email))
    .map((profile) => ({ brokerId: profile.id, name: profile.name, photoUrl: profile.photoUrl || "", contactCount: countByOwner.get(profile.id) || 0 }))
