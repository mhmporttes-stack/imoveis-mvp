...
export async function importProspectingContacts(rows, auth) {
  ...
  for (const row of Array.isArray(rows) ? rows.slice(0, 5000) : []) {
    const name = clean(row?.name || row?.Nome).slice(0, 160);
    const phone = toBrazilianE164(row?.phone || row?.whatsapp || row?.WhatsApp || row?.Telefone);
    if (!name || !phone) { summary.invalid += 1; continue; }
    const { data: existing, error: readError } = await db().from("prospecting_contacts").select("status").eq("phone_normalized", phone).maybeSingle();
    if (readError) throw readError;
    if (existing?.status === "do_not_contact") { summary.doNotContact += 1; continue; }
    if (existing) { summary.duplicates += 1; continue; }
    const { error } = await db().from("prospecting_contacts").insert({ name, phone_normalized: phone });
    if (error?.code === "23505") summary.duplicates += 1;
...
function buildGreetingUrl(contact) {
  ...
  return `https://wa.me/${contact.phone_normalized.replace(/\D/g, "")}?text=${encodeURIComponent(`${greeting}, ${firstName}`)}`;
}
