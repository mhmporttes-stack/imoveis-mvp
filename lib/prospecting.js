...
import { getDailyGoalWalletStatus, recordDoNotContactAudit } from "./daily-goal-wallet";
...
export async function claimProspectingContact(id, auth) {
  ...
  const { data: claimedRows, error } = await db().rpc("claim_single_prospecting_contact", { p_contact_id: id, p_broker_id: userId });
  if (error) {
  ...
  try {
    ...
    // Toda prospecção — pela fila manual (Base da Imobiliária/Minha Base) ou
    // pela Meta Diária — passa a entrar na mesma cadência de acompanhamento
    // de 2ª/3ª tentativa. Antes, só o que a Meta Diária atribuía sozinha
    // virava daily_goal_rounds; um contato clicado manualmente aqui nunca
    // ganhava rodada nenhuma e ficava perdido para sempre em relação a
    // follow-up (confirmado com um corretor real: dezenas de clientes
    // prospectados manualmente, nenhum com 2º/3º contato automático). O
    // clique em WhatsApp já conta como a 1ª mensagem (mesmo greeting que o
    // link abre), então a rodada nasce em attempt_count=1, pronta para
    // "2º contato" a partir de amanhã. distribution_type continua intocado —
    // não é o mesmo conceito da roleta de leads.
    const { error: roundError } = await db().from("daily_goal_rounds").insert({
      prospecting_contact_id: contact.id,
      client_id: registration.id,
      broker_id: userId,
      round_started_at: getTodayInSaoPaulo(),
      attempt_count: 1,
      status: "active"
    });
    if (roundError) console.warn("Não foi possível criar a rodada de acompanhamento da prospecção manual:", roundError.message || roundError);
...
export async function handleProspectingClientAction(registrationId, action, auth, payload = {}) {
  ...
  if (action === "return_to_queue") {
    ...
    const { error: registrationError } = await db().from("simulation_registrations").update({ responsible_user_id: null, status: CLIENT_STATUS.ARCHIVED, prospecting_assigned_pending: false, prospecting_assigned_by_user_id: null, last_status_change_at: now.toISOString() }).eq("id", registrationId);
    if (registrationError) throw registrationError;
