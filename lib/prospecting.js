...
export async function listProspectingContacts(auth) {
  ...
  if (!isOwnerAdminEmail(auth?.user?.email) && !isOwnerAdminEmail(auth?.profile?.email)) {
    // "recent_attempt" só entra na lista quando o prazo de espera (available_after)
    // já passou — do contrário outro corretor veria o contato e poderia mandar
    // WhatsApp de novo antes da hora, mesmo sem conseguir "reivindicar" ele.
