...
async function resolveCsvRecipients(rows) {
  ...
  for (const row of rows) {
    ...
    const phone = toBrazilianE164(rawPhone);
    if (!name || !phone) { invalidCount += 1; continue; }
    if (seenPhones.has(phone)) { duplicateCount += 1; continue; }
  ...
  const phones = candidates.map((row) => row.phone);
  ...
  if (error) throw error;
  const blockedSet = new Set((blocked || []).map((row) => row.phone_normalized));

  const valid = candidates.filter((row) => !blockedSet.has(row.phone));
...
export async function processBroadcastQueueBatch(broadcastId, maxDurationMs = 25000) {
  ...
  for (;;) {
    ...
    const { data: next, error } = await db().from("whatsapp_broadcast_messages")
      .select("*")
      ...
      .maybeSingle();
    if (error) throw error;
    if (!next) break; // fila deste broadcast esvaziou

    await db().from("whatsapp_broadcast_messages").update({ status: "processing" }).eq("id", next.id).eq("status", "queued");
...
async function sendSingleBroadcastMessage(message) {
  ...
  if (broadcastError || !broadcast) return;
  ...
  try {
    ...
    await db().from("whatsapp_broadcast_messages").update({
      status: "sent",
    ...
    if (classification === "temporary" && attempts < MAX_ATTEMPTS) {
      ...
      await db().from("whatsapp_broadcast_messages").update({
        ...
        error_message: String(sendError?.message || "").slice(0, 500)
      }).eq("id", message.id);
    } else {
      await db().from("whatsapp_broadcast_messages").update({
        status: "failed",
...
export function formatWhatsappBroadcastError(error) {
  ...
  if (normalized.includes("does not exist") && (normalized.includes("whatsapp_broadcast") || normalized.includes("whatsapp_templates"))) {
    return "As tabelas do Disparo ainda não existem no Supabase. Execute a migration supabase/migrations/20260920_whatsapp_master_disparo.sql.";
  }
