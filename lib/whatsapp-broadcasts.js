...
export async function syncTemplatesFromMeta(auth) {
  assertGeneralAdminOrManager(auth);
  await runTemplateSyncFromMeta();
  return listLocalTemplates(auth);
}

// Mesma sincronização, SEM checagem de usuário: só para o cron diário (rota protegida por token). Consultar a lista de
// modelos na Meta não gera custo — só envio de mensagem é cobrado.
export async function syncTemplatesFromMetaForCron() {
  const synced = await runTemplateSyncFromMeta();
  return { synced };
}

async function runTemplateSyncFromMeta() {
...
export async function getBroadcastDetail(id, auth) {
  ...
  // Quem respondeu ao disparo (1ª mensagem recebida em até 7 dias) e em qual conversa — para o Chat > Campanhas.
  const repliesByMessage = new Map();
  try {
    const { data: replies } = await db().rpc("whatsapp_broadcast_replies", { p_broadcast_id: id });
    for (const reply of replies || []) repliesByMessage.set(reply.broadcast_message_id, reply);
  } catch (repliesError) {
    console.warn("Falha ao carregar as respostas do disparo:", repliesError?.message || repliesError);
  }
  ...
  return {
    ...
    messages: (messages || []).map((row) => ({
      ...
      replied: repliesByMessage.has(row.id),
      repliedAt: repliesByMessage.get(row.id)?.replied_at || null,
      conversationId: repliesByMessage.get(row.id)?.conversation_id || null
    }))
  };
}
