...
export async function updateAdminProfile(id, payload = {}) {
  ...
  if ("leadDistributionEnabled" in payload) {
    updates.lead_distribution_enabled = nextRole !== ADMIN_ROLE.ASSOCIATE && payload.leadDistributionEnabled === true;
  }
...
function rowToAdminProfile(row = {}) {
  ...
  return {
    ...
    email,
    phone: row.phone || "",
    role: row.role === ADMIN_ROLE.ADMIN || isOwner ? ADMIN_ROLE.ADMIN : row.role === ADMIN_ROLE.ASSOCIATE ? ADMIN_ROLE.ASSOCIATE : ADMIN_ROLE.BROKER,
...
function normalizeAdminRole(role) {
  return [ADMIN_ROLE.ADMIN, ADMIN_ROLE.ASSOCIATE].includes(role) ? role : ADMIN_ROLE.BROKER;
}
