...
export function isGeneralAdminProfile(profile) {
  return profile?.role === ADMIN_ROLE.ADMIN;
}

export function isBrokerProfile(profile) {
  return profile?.role === ADMIN_ROLE.BROKER;
}

export function isActiveAdminProfile(profile) {
  return profile?.status === ADMIN_USER_STATUS.ACTIVE;
}
...
function rowToAdminProfile(row = {}) {
  return {
    ...
    phone: row.phone || "",
    role: row.role === ADMIN_ROLE.ADMIN ? ADMIN_ROLE.ADMIN : ADMIN_ROLE.BROKER,
    status: row.status === ADMIN_USER_STATUS.INACTIVE ? ADMIN_USER_STATUS.INACTIVE : ADMIN_USER_STATUS.ACTIVE,
