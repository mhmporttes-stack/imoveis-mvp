...
import { markRegistrationSimulationCompleted } from "./simulation-registrations";
...
export async function createSimulation(payload, userEmail = "") {
  const supabase = getSimulationClient();
  const record = simulationToRecord(payload, userEmail);
...
export async function updateSimulation(id, payload, userEmail = "") {
  ...
  if (error) throw error;
  await replaceSimulationProperties(data.id, payload.properties || []);
  if (payload.registrationId && simulationHasMeaningfulValues(payload)) {
    await markRegistrationSimulationCompleted(payload.registrationId);
  }
  return getSimulation(data.id);
}
