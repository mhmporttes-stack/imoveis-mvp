...
export async function getSimulation(id) {
  ...
  try {
    return {
      ...simulation,
      registration: await getSimulationRegistration(simulation.registrationId)
    };
  } catch {
    return simulation;
  }
...
export async function createSimulation(payload, userEmail = "") {
  const supabase = getSimulationClient();
  const payloadWithRegistration = await attachManualRegistration(payload);
  const record = simulationToRecord(payloadWithRegistration, userEmail);
...
export async function updateSimulation(id, payload, userEmail = "") {
  ...
  if (payloadWithRegistration.registrationId && simulationHasMeaningfulValues(payloadWithRegistration)) {
    await markRegistrationSimulationCompleted(payloadWithRegistration.registrationId);
  }
  return getSimulation(data.id);
}
...
async function attachManualRegistration(payload = {}) {
  if (payload.registrationId) return payload;
...
async function findSimulationByRegistration(registration) {
  ...
  const phone = String(registration?.phoneNormalized || registration?.phone || "").replace(/\D/g, "");
  const name = String(registration?.fullName || "").trim();
  ...
  return (data || [])
    ...
    .find((simulation) => {
      const notePhone = String(simulation.internalNote || "").replace(/\D/g, "");
      return (phone && notePhone.includes(phone)) || (name && simulation.clientName === name);
    }) || null;
}
