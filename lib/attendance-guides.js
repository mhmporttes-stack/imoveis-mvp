...
export async function getGuideSession({ conversationId, guideId = "" }, auth) {
  ...
  // O cliente já preencheu o formulário (últimos 3 dias)? Então o corretor NÃO pede o link de novo.
  let formFilledAt = null;
  try {
    const recentForm = await findRecentFormRegistration(conversation.contact_phone, 3 * 24 * 60 * 60 * 1000);
    formFilledAt = recentForm?.created_at || null;
  } catch {
    formFilledAt = null;
  }
  ...
  return {
    ...
    broker: { name: auth?.profile?.name || "", simulationLink: safeSimulationLink(auth?.profile), vars: buildBrokerGenderVars(auth?.profile) },
    formFilledAt,
