...
export const CONDITION_KINDS = ["business_hours", "is_client", "has_broker", "ad_origin", "has_name"];
export const TRIGGER_TYPES = ["keyword", "first_message", "ad_referral", "any_message"];

export const START_NODE_ID = "start";

// Link de simulação que abre DIRETO o formulário (sem a tela de escolha com
// "Atendimento rápido") — ver ?jornada=simulacao em LinkJourneyGate.jsx. O
// link pessoal do corretor (?ref=) continua o mesmo, só ganha o parâmetro.
export function directSimulationLink(link) {
  try {
...
export function interpolate(text, vars = {}) {
  let emptied = false;
  const result = String(text || "").replace(/\{\{\s*([a-zA-Z0-9_]+)\s*\}\}/g, (_, key) => {
  ...
  // Variável vazia (ex.: contato sem nome): "Olá, !" vira "Olá!" e sobra de
  // espaço some — só quando alguma variável ficou vazia.
  return emptied
    ? result.replace(/,\s*([!?.:;])/g, "$1").replace(/[ \t]+([!?.,:;])/g, "$1").replace(/[ \t]{2,}/g, " ").replace(/\n[ \t]+/g, "\n")
    : result;
}
...
export async function runFlow({ graph, flowId, session: initial, input = null, deps }) {
  ...
  while (pointer) {
    ...
    if (node.type === "condition") {
      const ok = Boolean(await deps.evaluateCondition(node.data, session.vars));
