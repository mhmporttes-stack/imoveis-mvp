export const CLIENT_STATUS = {
  // Cliente que veio pelo WhatsApp e ainda NÃO preencheu o formulário: só interagiu com as
  // automações do Chat. Passa para "Em atendimento" quando um corretor responde e para
  // "Aguardando simulação" quando preenche o link do formulário.
...
export const CLIENT_STATUS_OPTIONS = [
  ...
  { key: CLIENT_STATUS.AUTOMATED_SERVICE, value: CLIENT_STATUS.AUTOMATED_SERVICE, label: "Atendimento automático" },
  ...
  { key: CLIENT_STATUS.MEETING_PENDING, value: CLIENT_STATUS.MEETING_PENDING, label: "Aguardando reunião" },
  { key: CLIENT_STATUS.MEETING_DONE, value: CLIENT_STATUS.MEETING_DONE, label: "Reunião realizada" },
...
export const CLIENT_FUNNEL_SALE_STATUS_VALUES = [
  CLIENT_STATUS.SALE_COMPLETED,
  CLIENT_STATUS.SALE_FORMS,
  CLIENT_STATUS.SALE_RESERVATION,
  CLIENT_STATUS.SALE_CONTRACT,
...
// Macroetapas do funil comercial (Reorganização 2026-09-12): 7 etapas fixas.
// "Prospecção" não entra aqui — é a base do funil (todo cliente novo do
// período), tratada separadamente em lib/performance-overview.js.
// Restrição/Blindagem/Reprovado NÃO tiram o cliente do funil: continuam
// pertencendo à macroetapa "Aguardando aprovação" (regra explícita do
// negócio — reprovar/restringir é uma situação dentro da etapa, não uma saída).
// Os 8 status legados de venda (Contrato/Engenharia/Formulários/Reserva/
// Assinatura Caixa/ITBI/Cartório/Pagamento) continuam válidos no banco por
// compatibilidade histórica, mas todos contam como a única macroetapa VENDA —
// não há mais subetapas depois de Venda.
export const CLIENT_FUNNEL_STAGES = [
  { key: "service", label: "Atendimento", statuses: [CLIENT_STATUS.IN_SERVICE] },
  { key: "simulation", label: "Simulação", statuses: [CLIENT_STATUS.COMPLETED, CLIENT_STATUS.SIMULATION_SENT] },
  { key: "documentation", label: "Aguardando documentação", statuses: [CLIENT_STATUS.DOCUMENTATION, CLIENT_STATUS.DOCUMENTS_PENDING] },
  { key: "approval", label: "Aguardando aprovação", statuses: [CLIENT_STATUS.APPROVAL_PENDING, CLIENT_STATUS.RESTRICTION, CLIENT_STATUS.SHIELDING, CLIENT_STATUS.REJECTED] },
  { key: "approved", label: "Cliente aprovado", statuses: [CLIENT_STATUS.APPROVED] },
...
export const CLIENT_FUNNEL_SALE_STATUSES = new Set(CLIENT_FUNNEL_SALE_STATUS_VALUES);

// Grupos de abas macro da tela Clientes (/admin/simulacoes). Movido de
// components/AdminSimulationList.jsx para cá para ser a fonte única também
// da consulta server-side da listagem (lib/simulation-list-query.js) — antes
// só existia como cópia local do componente, o que arriscaria as duas
// divergirem (mesma classe de bug documentada em lib/crm.js).
export const CLIENT_STATUS_FILTER_GROUPS = [
  { key: "all", label: "Todos", statuses: [] },
  { key: "prospecting", label: "Prospecção", statuses: [CLIENT_STATUS.AWAITING_RETURN] },
  // Atendimento automático (cliente do WhatsApp que ainda não preencheu o formulário) fica na aba
  // Atendimento: é um contato novo que espera a resposta de um corretor.
  { key: "service", label: "Atendimento", statuses: [CLIENT_STATUS.AUTOMATED_SERVICE, CLIENT_STATUS.IN_SERVICE] },
...
// além da única legada em scheduledActivityAt). Esta função combina as duas
// fontes num único "scheduledActivityAt efetivo" — a mais próxima pendente,
// priorizando futuras sobre atrasadas — para reaproveitar sem alteração as
// regras acima (isOverdueActivityClient, isAwaitingFutureActivityClient etc.):
// "sem atividade futura" só é verdade quando NENHUMA atividade válida (legada
// ou extra) existir.
export function mergeActivitySignal(client, extraActivities = []) {
  const candidates = [];

  const legacyAt = validTimestamp(client?.scheduledActivityAt);
  if (legacyAt !== null && !client?.scheduledActivityCompletedAt) candidates.push(legacyAt);

  for (const activity of extraActivities) {
    if (activity?.status !== "pending") continue;
    const at = validTimestamp(activity.scheduledActivityAt);
    if (at !== null) candidates.push(at);
  }

  if (!candidates.length) {
    return { ...client, scheduledActivityAt: null };
  }

  const now = Date.now();
  const future = candidates.filter((at) => at >= now);
  const chosen = future.length ? Math.min(...future) : Math.min(...candidates);
  return { ...client, scheduledActivityAt: new Date(chosen).toISOString(), scheduledActivityCompletedAt: null };
}
