...
// devolve ele sem criar outro nem consumir uma nova posição da roleta (evita
// duplo-clique/reenvio/retry de rede gerando dois cards), e sem alterar
// nenhum dado do cadastro já existente (entrada pública não deve conseguir
// sobrescrever um cliente existente).
export async function createQuickAttendanceRegistration(payload = {}) {
  ...
  if (existing?.id) {
    return ensureRegistrationPreferenceToken(existing);
  }
