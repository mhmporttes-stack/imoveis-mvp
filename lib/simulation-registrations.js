...
export async function updateSimulationRegistration(id, updates = {}, auth = null) {
  ...
  if (updates.scheduledActivityAt !== undefined) record.scheduled_activity_at = normalizeScheduledActivityAt(updates.scheduledActivityAt);
...
export function formatSimulationRegistrationError(error) {
  ...
  if (
    ...
    return "Os campos de atividade agendada ainda não existem no Supabase. Execute a migration supabase/migrations/20260826_client_scheduled_activities.sql no SQL Editor do Supabase.";
  }
...
function registrationToRecord(registration = {}) {
  ...
  if (registration.scheduledActivityAt !== undefined) record.scheduled_activity_at = normalizeScheduledActivityAt(registration.scheduledActivityAt);
