...
import { canUseProfileDatabaseScope, isBrokerSchemaError, resolveAdminProfileByRef } from "./admin-profiles";
...
async function resolveResponsibleUserIdFromPayload(payload = {}, type = "simulation", auth = null) {
  if (auth?.profile?.role === "broker" && canUseProfileDatabaseScope(auth.profile)) {
