...
import { applyResponsibleUserScope, assertCanAccessResponsibleUser, assertGeneralAdminOrManager } from "./admin-access";
import {
...
export async function createSimulationRegistration(payload) {
  ...
  // Gerador de Links: um cadastro que chega via link de campanha (?c=<id>)
  // tem prioridade sobre o mecanismo de ref (?ref=) já existente — mas só
  // quando a campanha está ativa. Campanha inexistente ou desativada cai no
  // fluxo padrão de sempre, como se não tivesse vindo de nenhum link.
