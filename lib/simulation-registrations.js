...
async function findMatchingRegistration(draft) {
  ...
  const draftPhone = digitsOnly(draft.phoneNormalized);
  const draftName = normalizePersonName(draft.fullName).toLowerCase();

  return registrations.find((registration) => {
    const phone = digitsOnly(registration.phoneNormalized || registration.phone);
    return draftPhone && phone === draftPhone;
