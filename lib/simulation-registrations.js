...
export async function markRegistrationSimulationCompleted(id) {
  ...
  if (
    !current ||
    current.status === CLIENT_STATUS.APPROVED ||
    current.status === CLIENT_STATUS.DOCUMENTATION ||
    current.status === CLIENT_STATUS.ARCHIVED
  ) {
    return current;
  }
...
export async function markSimulationRegistrationAdminActivity(id, adminEmail = "") {
  const email = normalizeAdminEmail(adminEmail);
  if (!email) return getSimulationRegistration(id);
...
export async function markSimulationRegistrationWhatsAppContact(id, adminEmail = "") {
  const email = normalizeAdminEmail(adminEmail);
