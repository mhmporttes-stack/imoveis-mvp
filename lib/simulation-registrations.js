...
export async function createSimulationRegistration(payload) {
  ...
  const isTeamDistribution = normalizeBrokerRef(payload.brokerRef || payload.ref || payload.responsibleRef) === "equipe";
  const duplicateDraft = { ...validation.data };
  if (isTeamDistribution) {
    const existing = await findMatchingRegistration(duplicateDraft, {});
    if (existing?.id) return ensureRegistrationPreferenceToken(existing);
  }

  const responsibleUserId = await resolveResponsibleUserIdFromPayload(payload, "simulation");
  const existing = await findMatchingRegistration(duplicateDraft, {});
  if (existing?.id) {
    if (existing.responsibleUserId && existing.responsibleUserId !== responsibleUserId) {
  ...
  if (error?.code === "23505" && String(error?.message || "").includes("phone_normalized")) {
    const concurrentRegistration = await findMatchingRegistration(duplicateDraft, {});
    if (concurrentRegistration?.id) return ensureRegistrationPreferenceToken(concurrentRegistration);
  }
...
export async function updateSimulationRegistration(id, updates = {}, auth = null) {
  ...
  if (enteredSalePipeline) {
    try {
      const saleDate = new Intl.DateTimeFormat("en-CA", { timeZone: "America/Sao_Paulo" }).format(new Date(statusChangedAt));
