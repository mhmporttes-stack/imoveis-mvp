...
import { CAMPAIGN_DESTINATION, CAMPAIGN_STATUS, getCampaign, getOfficialCampaignForBroker } from "./campaigns";
...
async function resolveExternalLinkAssignment(payload = {}) {
  ...
  if (isActiveCampaign && campaign.destinationType === CAMPAIGN_DESTINATION.BROKER) {
    ...
    const { data: assignedBrokerId, error: assignError } = await getSupabaseAdminClient().rpc("assign_round_robin_lead");
    if (assignError) throw assignError;
    if (!assignedBrokerId) throw new Error("Nenhum corretor está ativo na distribuição de leads.");
    responsibleUserId = assignedBrokerId;
    distributionType = "round_robin";
    isRouletteAssignment = true;
  } else {
    responsibleUserId = await resolveResponsibleUserIdFromPayload(payload, "simulation");
  ...
  return { campaign: officialCampaign || campaign, isActiveCampaign, isTeamDistribution, providedRef, isDirectBrokerLink, responsibleUserId, distributionType, isRouletteAssignment };
}
...
async function finalizeExternalClientOrigin(registration, assignment) {
  const { campaign, isActiveCampaign, isRouletteAssignment, responsibleUserId } = assignment;

  if (isRouletteAssignment) {
    try {
      await recordLeadDistributionHistory({ registration, toUserId: responsibleUserId, eventType: "assigned" });
    } catch (historyError) {
...
export async function createSimulationRegistration(payload) {
  ...
  // Cliente que já tem cadastro (mesmo telefone) e preenche o formulário de
  // novo NÃO pode virar um segundo cadastro duplicado — atualiza o cadastro
  // existente com os dados novos. Só os dados do formulário em si (nome,
  // telefone, renda, estado civil etc.) são atualizados: responsável e
  // origem original NUNCA mudam aqui (isso é edição, não transferência — a
  // origem, aliás, é imutável no banco por trigger, mesma regra usada em
  // todo o resto do CRM) — só uma transferência explícita muda o responsável.
  ...
  if (existing?.id) {
    const updated = await updateSimulationRegistration(existing.id, validation.data, null);
...
export async function createQuickAttendanceRegistration(payload = {}) {
  ...
  if (existing?.id) {
    const updated = await updateSimulationRegistration(existing.id, { fullName, phone: payload.phone }, null);
    return ensureRegistrationPreferenceToken(updated);
  }
...
export async function ensureManualSimulationRegistration(payload = {}, auth = null) {
  ...
  const { data, error } = await supabase
    ...
    .insert(registrationToRecord({ ...draft, status: payload.status || CLIENT_STATUS.PENDING, adminEmail: payload.adminEmail,
      acquisitionContext: { kind: "manual", label: `Cadastro manual — ${auth?.profile?.name || payload.adminEmail || "Equipe"}`, actor: auth?.user?.email || payload.adminEmail || null, destination: "broker" }
    }))
    .select("*")
    .single();
...
async function resolveResponsibleUserIdFromPayload(payload = {}, type = "simulation", auth = null) {
  ...
  if (normalizeBrokerRef(ref) === "equipe") {
    const { data, error } = await getSupabaseAdminClient().rpc("assign_round_robin_lead");
    if (error) throw error;
    if (!data) throw new Error("Nenhum corretor está ativo na distribuição de leads.");
    return data;
  }
