...
import { digitsOnly, formatBrazilianPhone, isSamePhone, toBrazilianE164 } from "./phone-utils";
import {
...
export async function createSimulationRegistration(payload, requestMetadata = {}) {
  ...
  const existing = await findMatchingRegistration(
    { fullName: validation.data.fullName, phoneNormalized: validation.data.phoneNormalized },
    { preferOpenChatCard: true, preferResponsibleUserId: isDirectBrokerLink ? responsibleUserId : "" }
  );
...
// Cadastro mínimo direto, SEM varrer a base atrás de "cliente parecido" (ensureManualSimulationRegistration
// lista tudo e ainda casa por nome — pesado e sujeito a juntar homônimos). Usado pelo WhatsApp quando o
// contato novo já tem quem atenda na conversa; quem chama garante antes que o telefone não é cliente.
export async function createDirectContactRegistration({ fullName, phone, responsibleUserId, acquisitionContext }) {
  const draft = { ...buildManualSimulationRegistration({ fullName, phone, includeDetails: false }), responsibleUserId };
  if (!draft.fullName) throw new Error("Informe o nome do cliente para criar o cadastro.");
  if (!draft.phoneNormalized) throw new Error("Informe um WhatsApp valido do cliente para criar o cadastro.");

  const supabase = getSimulationRegistrationsClient();
  const { data, error } = await supabase
    .from("simulation_registrations")
    // Já tem quem atenda na conversa (um corretor respondeu): "Em atendimento", não "Aguardando
    // simulação" — o cliente ainda não preencheu o formulário.
    .insert(registrationToRecord({ ...draft, status: CLIENT_STATUS.IN_SERVICE, acquisitionContext }))
    .select("*")
    .single();
  if (error) throw error;
  try { await syncCrmClientAttendance(data); } catch (syncError) { console.error("CRM client/attendance sync failed:", syncError?.message || syncError); }
  // Nasce "Em atendimento" (corretor já respondeu): registra a mudança no histórico, no nome de
  // quem atende — mesmo marco de atendimento de qualquer cadastro criado nesse status.
  try {
    const { data: attendant } = responsibleUserId ? await supabase.from("admin_users").select("email").eq("id", responsibleUserId).maybeSingle() : { data: null };
    await recordClientStatusChange({ supabase, clientId: data.id, previousStatus: null, newStatus: data.status, changedAt: data.last_status_change_at || data.created_at, changedBy: attendant?.email || "sistema", source: "whatsapp_chat" });
  } catch (historyError) {
    console.warn("Falha ao registrar o status inicial do contato do WhatsApp:", historyError?.message || historyError);
  }
  return rowToSimulationRegistration(data);
}
...
export function rowToSimulationRegistration(row = {}) {
  ...
  const registration = {
    ...
    // Tipo da origem (ex.: whatsapp_ad, whatsapp_chat, broker_link) — só o "kind", nunca o contexto inteiro.
    acquisitionKind: typeof row.acquisition_context?.kind === "string" ? row.acquisition_context.kind : "",
    tags,
...
async function findMatchingRegistration(draft, options = {}) {
  ...
  if (phoneMatches.length) {
    ...
    if (options.preferOpenChatCard) {
      const openChatCard = phoneMatches.find((registration) => (
        String(registration.acquisitionKind || "").startsWith("whatsapp") && !hasSimulationData(registration)
      ));
...
// Card criado pelo WhatsApp (anúncio/Chat) que o cliente ainda não completou com a simulação:
// o formulário de qualquer link o COMPLETA (mantendo o responsável atual) em vez de abrir outro card.
function isOpenChatCard(registration) {
  return Boolean(registration) && String(registration.acquisitionKind || "").startsWith("whatsapp") && !hasSimulationData(registration);
}
