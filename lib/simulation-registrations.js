...
import { CAMPAIGN_DESTINATION, CAMPAIGN_STATUS, getCampaign, recordClientOrigin } from "./campaigns";
...
export async function createSimulationRegistration(payload) {
  ...
  if (campaign) {
    try {
      await recordClientOrigin({
        clientId: registration.id,
        campaignId: campaign.id,
        campaignNameSnapshot: campaign.name
      });
    } catch (originError) {
      console.warn("Client origin recording failed:", originError?.message || originError);
    }
  }
