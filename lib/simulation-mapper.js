...
export function rowToSimulation(row, properties = []) {
  ...
  return {
    ...
    clientName: normalizePersonName(row.client_name || ""),
    ...
    autosaveUpdatedAt: row.autosave_updated_at || "",
    entrySimulationSnapshots: Array.isArray(row.entry_simulation_snapshots) ? row.entry_simulation_snapshots : [],
    properties
  };
}
...
export function simulationToRecord(simulation, userEmail = "") {
  ...
  const record = {
    client_name: normalizePersonName(simulation.clientName) || "Cliente sem nome",
  ...
  if (Array.isArray(simulation.entrySimulationSnapshots)) {
    record.entry_simulation_snapshots = simulation.entrySimulationSnapshots;
  }

  if (text(simulation.registrationId)) {
    record.registration_id = text(simulation.registrationId);
  }

  if (text(simulation.createdByUserId)) {
    record.created_by_user_id = text(simulation.createdByUserId);
  }
