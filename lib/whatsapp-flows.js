...
import { buildBrokerGenderVars } from "./broker-gender";
import { addTagToClient } from "./client-tags";
import { canonicalWhatsappPhone } from "./phone-utils";
import { findConversationByPhone, findRecentFormRegistration } from "./client-phone-lookup";
import { filterFlowsForPostFormContact, isFormCompletionMessage } from "./whatsapp-form-completion.mjs";
...
const HUMAN_ATTENDING_MS = 30 * 60 * 1000;
...
// Conversa em QUALQUER formato do número (com/sem 9º dígito) — estratégia central de telefone.
async function loadConversationByPhone(phone) {
  return findConversationByPhone(phone, "id, contact_phone, contact_name, client_id, status, last_inbound_at, origin");
}
...
async function buildBaseContext(conversation) {
  ...
  if (ctx.responsibleId) {
    broker = await getAdminProfileById(ctx.responsibleId).catch(() => null);
    if (broker) {
  ...
  ctx.vars = {
    ...
    link_simulacao: directSimulationLink(link),
    corretor: brokerName,
    ...(broker ? buildBrokerGenderVars(broker) : {})
  };
  return ctx;
}
...
function makeDeps({ session, conversation, ctx, logs, flowId }) {
  ...
  return {
    ...
    evaluateCondition: async (data, vars = {}) => {
      switch (data.kind) {
        case "has_name":
          return String(vars.nome || "").trim().length > 0;
        case "business_hours":
    ...
    runActions: async (actions, vars) => {
      ...
      for (const action of actions) {
        if (action.type === "roulette") {
          ...
          if (!ctx.clientId) {
            const outcome = await materializeClientFromAutomationReply({
              phone,
              name: vars.nome || conversation.contact_name || "",
              conversationId: conversation.id,
              via: "flow",
              details: { flowId }
            });
            brokerProfile = outcome.brokerProfile || null;
            ...
            result.detail.roleta = outcome.registrationId ? (outcome.alreadyExisted ? "cliente_ja_existia" : "cliente_criado") : "sem_corretor_disponivel";
          } else {
            result.detail.roleta = "cliente_ja_existia";
          }

          // Cliente que já existia: o corretor é o responsável dele.
          if (!brokerProfile && ctx.clientId) {
            const { data: registration } = await db().from("simulation_registrations").select("responsible_user_id").eq("id", ctx.clientId).maybeSingle();
            if (registration?.responsible_user_id) brokerProfile = await getAdminProfileById(registration.responsible_user_id).catch(() => null);
          }

          if (brokerProfile) {
            result.vars.corretor = brokerProfile.name || "";
            Object.assign(result.vars, buildBrokerGenderVars(brokerProfile));
            result.vars.link_simulacao = directSimulationLink(buildBrokerSimulationLink(brokerProfile));
            ctx.responsibleId = brokerProfile.id || ctx.responsibleId;
            result.detail.corretor = brokerProfile.name || "";
            // A conversa já fica com o corretor sorteado (ele vê no Chat e assume) e ele é avisado.
            await db().from("whatsapp_conversations").update({ assigned_user_id: brokerProfile.id, updated_at: new Date().toISOString() }).eq("id", conversation.id).is("assigned_user_id", null);
            await db().from("crm_notifications").insert({
              recipient_user_id: brokerProfile.id,
              client_id: ctx.clientId,
              title: "Novo atendimento no WhatsApp",
              description: `${vars.nome || conversation.contact_name || "Um cliente"} quer falar com um corretor. Abra o Chat e assuma o atendimento.`,
              notification_type: "automation",
              scheduled_at: new Date().toISOString()
            }).then(({ error }) => { if (error) console.warn("Falha ao notificar o corretor:", error.message); });
          }
...
export async function processFlowInbound(event) {
  ...
  const live = await loadLiveSession(conversation.contact_phone);
  if (live) {
  ...
  const { count: recentHumanMessages } = await db()
    ...
    .eq("conversation_id", conversation.id)
    .eq("sender_type", "user")
    .neq("direction", "internal") // mensagem interna não é resposta ao cliente
    .gt("message_at", new Date(Date.now() - HUMAN_ATTENDING_MS).toISOString());
  const humanIsAttending = (recentHumanMessages || 0) > 0;

  let matching = flows.filter((flow) => matchTrigger(flow.published_trigger, context));
  // Contato pós-formulário (botão "Receber minha simulação" ou cadastro recente por formulário/link):
  // o cliente já tem corretor — só o fluxo dedicado "Formulário concluído" trata a mensagem do botão,
  // e gatilhos automáticos (qualquer mensagem, primeira mensagem, anúncio) não disparam.
  if (matching.length) {
    const isFormMessage = isFormCompletionMessage(text);
    const recentForm = isFormMessage ? null : await findRecentFormRegistration(phone);
    const allowed = filterFlowsForPostFormContact(matching, { isFormMessage, recentForm: Boolean(recentForm) });
    const suppressed = matching.filter((flow) => !allowed.includes(flow));
    if (suppressed.length) {
      await db().from("whatsapp_flow_logs").insert(suppressed.map((flow) => ({
        flow_id: flow.id,
        contact_phone: phone,
        kind: "skipped",
        detail: { reason: "o cliente acabou de preencher o formulário (contato pós-formulário)", text: text.slice(0, 120) }
      })));
    }
    matching = allowed;
  }
  // Uma pessoa acabou de responder este cliente: só palavra-chave (pedido
  // explícito do cliente) pode iniciar um fluxo por cima. O motivo fica no
  // histórico do fluxo (Atividade) para não parecer que "não funcionou".
  const blocked = humanIsAttending ? matching.filter((flow) => flow.published_trigger?.type !== "keyword") : [];
  if (blocked.length) {
    await db().from("whatsapp_flow_logs").insert(blocked.map((flow) => ({
      flow_id: flow.id,
      contact_phone: phone,
      kind: "skipped",
      detail: { reason: "um atendente respondeu este cliente há menos de 30 minutos", text: text.slice(0, 120) }
    })));
  }
  const candidates = matching
    .filter((flow) => !blocked.includes(flow))
