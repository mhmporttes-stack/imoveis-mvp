...
export async function processFlowInbound(event) {
  ...
  const candidates = flows
    ...
    // Uma pessoa já está atendendo: só palavra-chave (pedido explícito do
    // cliente) pode iniciar um fluxo por cima.
    .filter((flow) => conversation.status !== "in_service" || flow.published_trigger?.type === "keyword")
