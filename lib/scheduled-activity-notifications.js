...
async function sendPushReminder(registration) {
  ...
  const result = await sendPushToUser(registration.responsibleUserId, {
    ...
    url: buildAdminRegistrationUrl(registration.id) || "/admin/simulacoes"
  });
...
function buildAdminRegistrationUrl(id) {
  // getSiteBaseUrl() sempre resolve para uma URL absoluta válida (com
  // fallback para o domínio de produção) — evitar repetir aqui a checagem
  // frágil de env vars que, quando NEXT_PUBLIC_SITE_URL/VERCEL_URL faltavam,
  // fazia essa função retornar "" e o push/e-mail caíam no link genérico do
  // painel em vez de abrir o cadastro do cliente.
  return `${getSiteBaseUrl()}/admin/cadastros/${id}`;
}
