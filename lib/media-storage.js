...
// Mídias enviadas pelo Chat do WhatsApp (imagens, áudios, documentos e os
// atalhos). Público porque a Meta baixa o arquivo pelo link; os caminhos têm
// UUID (não adivinháveis).
export const CHAT_MEDIA_BUCKET = process.env.SUPABASE_CHAT_MEDIA_BUCKET || "whatsapp-chat-media";
...
export async function uploadChatMedia({ buffer, contentType, fileName, folder = "sent" }) {
  const supabase = getSupabaseAdminClient();
  if (!supabase) throw new Error("Supabase Storage nao configurado.");

  if (!chatMediaBucketReady) {
    const { data: buckets, error: listError } = await supabase.storage.listBuckets();
    if (listError) throw new Error(listError.message || "Nao foi possivel verificar os buckets do Supabase Storage.");
    if (!buckets?.some((bucket) => bucket.name === CHAT_MEDIA_BUCKET)) {
      const { error } = await supabase.storage.createBucket(CHAT_MEDIA_BUCKET, { public: true, fileSizeLimit: 10_000_000 });
      if (error && !String(error.message || "").toLowerCase().includes("already exists")) {
        throw new Error(error.message || "Nao foi possivel criar o bucket de midias do Chat.");
      }
    }
    chatMediaBucketReady = true;
  }

  const extension = extensionFromType(contentType) || extensionFromName(fileName) || "bin";
  const path = `${safePathPart(folder)}/${Date.now()}-${randomUUID()}-${safeFileName(fileName, extension)}`;
  const { error } = await supabase.storage.from(CHAT_MEDIA_BUCKET).upload(path, buffer, {
    cacheControl: "31536000",
    contentType: contentType || "application/octet-stream",
    upsert: false
  });
  if (error) throw new Error(error.message || "Nao foi possivel enviar o arquivo para o Supabase Storage.");
