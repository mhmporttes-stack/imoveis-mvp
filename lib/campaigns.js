...
export async function listCampaigns() {
  ...
  const [brokerNames, viewCounts] = await Promise.all([
    getBrokerNamesByIds(brokerIds),
    countViewsByCampaign((campaigns || []).map((row) => row.id))
  ]);
...
// count:"exact",head:true por campanha (não um .select() bruto de todas as
// linhas) — mesma correção já aplicada em listBrokerBasesSummary
// (lib/prospecting.js) para o limite implícito de 1000 linhas do PostgREST;
// aberturas de link crescem mais rápido que conversões, então essa tabela é
// a mais propensa a estourar esse limite.
async function countViewsByCampaign(campaignIds = []) {
  if (!campaignIds.length) return {};
  const supabase = getCampaignsClient();
  const counts = await Promise.all(
    campaignIds.map((id) => supabase.from("campaign_link_views").select("id", { count: "exact", head: true }).eq("campaign_id", id))
  );
