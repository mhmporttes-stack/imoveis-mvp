...
export async function listCampaigns({ period, startDate, endDate, type = "all", search = "" } = {}) {
  ...
  const [viewCounts, originsByCampaign] = await Promise.all([
  ...
  }), { views: 0, clients: 0, simulation: 0, sale: 0 });
  ...
  return {
    ...
    summary: {
      ...summary,
      conversion: summary.views ? summary.clients / summary.views : null
    }
  };
}
...
export async function getCampaign(id) {
  ...
  const brokerInfo = data.broker_id ? await getBrokerInfoByIds([data.broker_id]) : {};
