...
async function runScopedQuery(auth, columns, build) {
  ...
  if (scope.all) {
    const { data, error } = await build(db().from("whatsapp_conversations").select(columns));
    if (error) throw error;
    return data || [];
  }
  const [byClient, byAssignee] = await Promise.all([
    build(db().from("whatsapp_conversations").select(`${columns}, ${SCOPE_EMBED}`)).in("scope.responsible_user_id", scope.brokerIds),
    build(db().from("whatsapp_conversations").select(columns)).in("assigned_user_id", scope.brokerIds)
  ]);
...
function messageRow(row, auth) {
  ...
  return {
    ...
    media: row.metadata?.media?.url ? { url: row.metadata.media.url, mime: row.metadata.media.mime || "", name: row.metadata.media.name || "" } : null,
