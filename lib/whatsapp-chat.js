...
import { markClientTryingContact, markClientsInServiceOnReply } from "./whatsapp-client-status";
...
export async function projectChatFromEvents(events, clientByPhone = new Map()) {
  ...
  if (inbound.length) {
    ...
    if (messageRows.length) {
      ...
      for (const [conversationId, rows] of newByConversation) {
        ...
        if (repliedClientId && rows.some((row) => isClientReplyMessage(row.message_type))) repliedClientIds.add(repliedClientId);
      }
    }
  }
  ...
  try {
    await markClientsInServiceOnReply([...repliedClientIds]);
  } catch (statusError) {
...
export async function markChatConversationRead(id, auth) {
  const conversation = await loadConversation(id, auth);
  if (chatScope(auth).all && conversation.assigned_user_id !== (auth?.profile?.id || null)) {
    return { marked: false };
  }
...
export async function updateChatConversationStatus(id, status, auth) {
  ...
  if (status === "in_service" && auth?.profile?.id) {
...
export async function sendChatMessage(id, text, auth) {
  ...
  await noteHumanContact(conversation, auth);
...
// Regra do dono: uma pessoa da equipe enviou mensagem ao cliente pelo Chat -> "Tentando contato"
// (só se ele ainda está em "Aguardando simulação"). Melhor esforço: a mensagem já foi enviada.
async function noteHumanContact(conversation, auth) {
  if (!conversation?.client_id) return;
  try {
    await markClientTryingContact(conversation.client_id, auth?.user?.email || auth?.profile?.email || "");
  } catch (statusError) {
    console.warn("Falha ao atualizar o status do cliente após a mensagem:", statusError?.message || statusError);
  }
}
