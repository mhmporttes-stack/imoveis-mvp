...
import { markClientOnHumanMessage, markClientsInServiceOnReply, startServiceOnManualAdd } from "./whatsapp-client-status";
...
export async function resolveChatMedia(messageId, auth, { retry = false, download = false } = {}) {
  ...
  if (error) throw error;
  if (!row || row.direction !== "inbound" || !INBOUND_MEDIA_TYPES.includes(row.message_type)) throw new WhatsappChatError("Arquivo não encontrado.", { status: 404 });
...
async function noteHumanContact(conversation, auth) {
  ...
  try {
    await markClientOnHumanMessage(conversation.client_id, auth?.user?.email || auth?.profile?.email || "");
  } catch (statusError) {
