...
// a análise em si.
export async function recordAiUsage({ feature = "client_document_analysis", model, batchId, clientId, triggeredBy, inputTokens = 0, outputTokens = 0, cacheCreationTokens = 0, cacheReadTokens = 0, costUsd = 0, success = true, errorMessage = null }) {
  try {
    await db().from("ai_usage_log").insert({
      ...
      cache_creation_input_tokens: Math.max(0, Math.round(cacheCreationTokens)),
      cache_read_input_tokens: Math.max(0, Math.round(cacheReadTokens)),
...
export async function getAiUsageReport(auth) {
  ...
  const { data: rows, error } = await db()
    .from("ai_usage_log")
    .select("id, feature, model, batch_id, client_id, triggered_by, input_tokens, output_tokens, cache_creation_input_tokens, cache_read_input_tokens, estimated_cost_usd, success, error_message, created_at")
