...
const MARITAL_LABELS = { married: "Casado(a)", single: "Solteiro(a)", divorced: "Divorciado(a)", stable_union: "União estável", widowed: "Viúvo(a)" };
...
async function recomputeRequirements(registration, clientContext, batchId, auth) {
  ...
  const { data: existingRequirementRows, error: existingError } = await db()
    .from("client_document_checklist_items")
    .select("person_label, document_type, corrected_by")
    .eq("client_id", registration.id)
    .is("document_id", null);
  if (existingError) throw existingError;
  const preservedKeys = new Set((existingRequirementRows || []).filter((row) => row.corrected_by).map((row) => `${row.person_label}::${row.document_type}`));

  await db().from("client_document_checklist_items").delete().eq("client_id", registration.id).is("document_id", null).is("corrected_by", null);
  ...
  const toInsert = computedRows.filter((row) => !preservedKeys.has(`${row.personLabel}::${row.documentType}`));

  if (toInsert.length) {
    const { error: insertError } = await db().from("client_document_checklist_items").insert(toInsert.map((row) => ({
      batch_id: batchId,
      client_id: registration.id,
      document_id: null,
...
export async function submitToCca(clientId, batchId, payload, auth) {
  ...
  if (missingFields.length) {
    const err = new Error("Preencha nome, e-mail e PIS do cliente antes de gerar o pacote para a CCA.");
