...
| **P-21** | ~~Código TEMPORÁRIO de testes na `main`~~ — **ENCERRADO** (2026-09-25). O código já havia sido removido no commit `9bcae0c` (`chore: remove rota e proteções temporárias de teste do Chat/Roleta/Áudio`). Existiam (commits `TEMP:` de 2026-09-24): trava de envio simulado para destinos `+5500…` em `lib/whatsapp-master.js` (`dryRunForFictionalRecipient`, ID `wamid.DRYRUN…`), pulo de push para conversas `+5500…` em `lib/whatsapp-chat.js` (`notifyInternalMessage`) e a rota `app/api/admin/tmp-chat-tests` (protegida por hash de segredo, rodava no banco de produção). Nenhuma função de produção dependia deles; `maxDuration = 30` do webhook permanece (legítimo). **Auditoria de dados (somente leitura, 2026-09-25):** o teste apagou os próprios clientes, conversas e mensagens, mas deixou **12 resíduos em `crm_clients`** (`+5500999…`, nome “TESTE CRITICO…”, sem vínculo em `crm_attendances`; tabela de compatibilidade só escrita pelo app, sem efeito em métricas/funil/ranking). **Os 12 foram removidos** (DELETE em transação, conferência prévia = 12) e a verificação final encontrou **0 resíduos conhecidos do teste** (`crm_clients`, `crm_attendances` órfãs, `simulation_registrations`, `whatsapp_conversations`, `whatsapp_messages` `DRYRUN`). **Não alterados:** 13 outros `crm_clients` órfãos com telefones reais (sem atendimento; origem não investigada) e os registros reais de `whatsapp_conversation_audit` (exclusões de conversa feitas pelo dono, telefones reais) | — | `lib/whatsapp-master.js`, `lib/whatsapp-chat.js`, `app/api/webhooks/whatsapp-master/route.js` |
