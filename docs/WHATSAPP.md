...
- **Status do cliente automático.** **[REGRA OFICIAL DE NEGÓCIO — definida pelo dono em 2026-09-25/26]** O status do CLIENTE muda sozinho conforme a conversa no Chat. **Cliente que veio pelo WhatsApp e ainda não preencheu o formulário** (anúncio, contato direto, palavra-chave, Fluxo — criados pela função de banco `whatsapp_get_or_create_roulette_client`) **nasce em “Atendimento automático”** (`automated_service`; só interagiu com as automações do Chat), nunca em “Aguardando simulação”. Quando uma **pessoa da equipe** responde/envia mensagem no Chat (texto, modelo, atalho ou anexo) ele passa para **“Em atendimento”**; quando **preenche o link do formulário** passa para **“Aguardando simulação”** (já feito por `createSimulationRegistration`, que reconhece o telefone). Cliente que **já preencheu o formulário** (“Aguardando simulação”): mensagem de uma pessoa da equipe → **“Tentando contato”**; o cliente **responde** (qualquer mensagem, menos reação) estando em “Tentando contato” → **“Em atendimento”**. Contato criado por conversa **já assumida** (`createDirectContactRegistration`) e conversa adicionada ao CRM por um corretor sem formulário preenchido (`startServiceOnManualAdd`) já ficam “Em atendimento”. Só anda para a frente: cliente em outra etapa, arquivado ou “não contactar” nunca é alterado; respostas automáticas (Fluxos/palavra-chave) não contam como “corretor enviou”; a resposta do cliente só promove quem tem corretor responsável. “Preencheu o formulário” = `hasSimulationData` (`lib/simulation-registration-schema.js`). Grava `client_status_history` (`source = whatsapp_chat`); “Em atendimento” automático fica no nome do **corretor responsável** (marco de atendimento da pontuação, uma vez por cliente) e “Tentando contato” no de quem enviou. “Em atendimento” encerra a rodada da Meta Diária desse cliente como conversão (regra já existente). “Atendimento automático” é status ativo, aparece na aba Atendimento da lista de clientes, não conta no funil (o funil só começa em “Em atendimento”) e sem tela pública própria (a Minha Jornada usa o texto padrão). Vale para mensagens novas; em 2026-09-26 os 8 clientes do WhatsApp que já estavam “Aguardando simulação”/“Tentando contato” sem formulário foram corrigidos (migration `20260926140000`, parte 3). — `lib/whatsapp-client-status-core.mjs`, `lib/whatsapp-client-status.js`, `lib/whatsapp-chat.js` (`noteHumanContact`, `projectChatFromEvents`, `addChatConversationToCrm`), `lib/client-status.js`, `tests/whatsapp-client-status.test.mjs`.

- **Situação do cliente no Chat**: a lista, o cabeçalho da conversa e o painel de informações mostram o **status do card do cliente** (mesmos rótulos/cores de `CLIENT_STATUS_META`, ex.: "Aguardando simulação", "Em atendimento"; `ClientStatusBadge`) e, no painel, se a **simulação foi preenchida** (`client.simulationFilled`, via `hasSimulationData`). Conversa sem cliente continua "Não cadastrado". — `lib/whatsapp-chat.js` (`loadClientInfo`), `components/WhatsappChatBadges.jsx`.
- **Cabeçalho da conversa** (`Thread` em `components/WhatsappChat.jsx`, só visual): linha 1 = foto, nome (o **telefone não aparece aqui**; fica no painel de informações), situação da janela de 24 h (ponto verde/âmbar) e status do card, ações "i" e "⋯"; linha 2 = corretor, status da conversa e sinais "no vácuo" (uma linha, rola de lado); no celular o "Assumir atendimento" é uma faixa larga (só se a conversa não é sua). **Finalizar/Reabrir e Excluir conversa ficam no menu "⋯"** (mesmas permissões e rotas de antes).
...
- **Gastos e Desempenho** (abas do Disparo — `lib/whatsapp-broadcast-finance.js`, `GET/PUT /api/admin/whatsapp-broadcasts/finance`, `components/WhatsappDisparoInsights.jsx`): a Meta informa em **cada evento de status** se a mensagem é cobrada e a categoria (`pricing.billable`/`category`); o webhook grava em `whatsapp_broadcast_messages.billable`/`pricing_category`. **Custo = mensagens ENTREGUES e cobradas × preço da categoria**; o preço em R$ é uma tabela editável (`crm_settings 'whatsapp_pricing'`, padrão **estimado** — faixas públicas da Meta no Brasil: marketing 0,35 · utilidade 0,05 · autenticação 0,17 — a conferir com a fatura; só o administrador geral edita). Tipo do disparo = categoria do modelo (marketing/utilidade/autenticação). **Custo por conversa** = custo total da campanha ÷ contatos que **responderam em até 7 dias** (1ª mensagem recebida após o envio); **custo por cadastro** = custo ÷ cadastros gerados pelo link da campanha (`client_origins.campaign_id`). Desempenho por campanha e por modelo.
- **Chat > Campanhas** (só admin/gestor; `components/WhatsappChatCampaigns.jsx`): lista todos os disparos e, ao abrir, os destinatários com status e quem respondeu (botão abre a conversa). **Destinatário de disparo não vira conversa**: só aparece em Conversas e na Visão geral quando responde (o envio do Disparo não cria conversa).
