...
| `whatsapp_phone_key`, `whatsapp_broadcast_replies(uuid)`, `whatsapp_broadcast_report(uuid[])` (migration `20260926120000`, só `service_role`) | chave de telefone (DDD + 8 últimos dígitos); quem respondeu ao disparo (1ª mensagem recebida em ≤ 7 dias); relatório por campanha (cobrança por categoria, respostas, aberturas do link, cadastros). Colunas novas `whatsapp_broadcast_messages.billable`/`pricing_category` (vindas do webhook) e `crm_settings.id='whatsapp_pricing'` (preços por mensagem) |
