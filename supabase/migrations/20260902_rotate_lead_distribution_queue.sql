...
begin
  select last_broker_id into current_last_id
  from public.lead_distribution_state
  where id = 'default';

  select coalesce(max(lead_distribution_position), 0) + 1 into last_position
  from public.admin_users;

  update public.admin_users
  set lead_distribution_position = last_position
...
with ordered as (
  ...
  from public.admin_users
  where status = 'active'
    and role in ('admin', 'broker', 'associate')
    and lead_distribution_enabled = true
)
