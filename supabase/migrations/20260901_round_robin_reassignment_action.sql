drop function if exists public.assign_round_robin_lead();
...
begin
  ...
  for candidate_offset in 1..array_length(broker_ids, 1) loop
    candidate_id := broker_ids[
      mod(coalesce(start_position, 0) + candidate_offset - 1, array_length(broker_ids, 1)) + 1
    ];
    if excluded_broker_id is null or candidate_id <> excluded_broker_id then
      exit;
    end if;
    candidate_id := null;
  end loop;
